...
- `cas`: add the `revision` content format (`fs/media/revision`) and `fs/cas/evo` — the first iteration of `fs/cas/todo/revision-content-format.md`. `fs/media/revision` is the pure format: an RTTI schema for a revision BLOB (`object`, `parents`, `content`, `changes`, `generation`, `archived`), the `application/vnd.functionalscript.revision+json` `mimeType` tag, and `ref`/`hash` semantic checks (`isHash`, `isHttpsRef`, `isRef`) layered on top of the RTTI shape validation, since a plain `string` schema can't express "a cbase32 hash or an `https://` URL". `fs/cas/evo` adds the store-touching operations over a `Cas<O>`: `heads` (per-`object` head resolution via a reverse parent index), `materialize` (first iteration: zero or one parent, no `changes` yet — a multi-parent merge revision must carry `content`), and `computeGeneration` (re-derives the cached `generation` from `parents` so a caller never has to trust an unverified value). Extracts `collectRead` (draining a `Cas` read stream into one `Vec`) from `fs/cas/mcp` into the shared `fs/cas/module.f.ts` now that `fs/cas/evo` is a second consumer. Adds `fs/cas/README.md` (didn't exist before) referencing the store's siblings and the new format. PR [#1252](https://github.com/functionalscript/functionalscript/pull/1252)
