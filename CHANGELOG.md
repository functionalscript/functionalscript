...
- `cas/mcp` / `text` / `base64` / `bit_vec`: reject oversized inline `cas_add` content as a clean tool error on every JS engine instead of crashing (`bun`) or silently storing an over-`maxLength` blob (`node`/`deno`). The boundary builders that ingest external input now return `Nullable<Vec>` (`null`) instead of building a `bigint` past the `maxLength` (128 KiB) ceiling: `u8ListToVec` in `fs/types/bit_vec/module.f.ts` and `utf8` in `fs/text/module.f.ts`; `fs/base64` `decode` keeps its `Nullable<Vec>` signature and now returns `null` (rather than throwing) for over-`maxLength` input, computing the decoded length up front and splitting off the trailing base64 char so a `maxLengthBytes` blob builds without overflow. The `cas_add` handler turns a `null` from `utf8` / `decode` into a generic content-decoding `isError` result that points oversized content at `type:'url'`. Pure combinators (`concat`, `listToVec`, …) stay total with a documented `len ≤ maxLength` precondition. Callers that have already bounded their input add `!`. New proofs cover inline `text` / `base64` at exactly `maxLengthBytes` (stored) and one byte over (clean error), plus the `u8ListToVec` size boundary (cas-add-inline-size-error)
