...
- `fjs`: `fjs r` now looks up `main` instead of `default` on the imported module; update `fs/ci/module.f.ts`, `fs/website/module.f.ts`, and `fs/dev/index/module.f.ts` from `export default` to `export const main` ([i667-fjs-run-main-convention](./issues/667-fjs-run-main-convention.md))
