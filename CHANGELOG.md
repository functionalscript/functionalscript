...
- `fjs/effects`: **BREAKING CHANGE:** an `Effect<O, T>` is now the **raw value**
  ...
  `.value`), mirroring `compose`/`fn` in `fjs/types/function`. `decode` and
  `step` are the only two functions that inspect an effect's shape — `step`
  reads it inline rather than through `decode` to avoid building a `Decoded`
  record per composed link; the `Do` continuation is extracted into
...
- `fs/` → `fjs/`: **BREAKING CHANGE:** rename the top-level source directory
  ...
  `functionalscript/fjs/asserts/module.js`. `fs` collided with Node's
  built-in `fs` module, while `fjs` is already the project's public verb (the
  npm `bin` and the `deno.json` task are both named `fjs`), so this aligns
  the source tree with the name users type and with FunctionalScript/`.f.ts`
  branding. Also moves the leftover `fs/fjs/todo/*` files (remaining after
  the `fjs` CLI was promoted to the `fs/` root) to `fjs/todo/`, removing the
  emptied `fjs/fjs/`. Cross-module imports are relative and untouched; updates
  `package.json` (`bin.fjs`, script paths), `deno.json` (the `fjs` task
  path), `package-lock.json`, and path references across the README and
  `todo/` files. No transition release with `fs/` re-export stubs is shipped.
  Implements and deletes `fjs/todo/rename-fs-to-fjs.md`
- `fs/media/revision`, `fs/cas/evo`: **BREAKING CHANGES:** make `snapshot` and `generation` required in `revisionSchema` (`option(hash)` → `hash`, `option(number)` → `number`), landing the format's required-fields rule before any `vnd.fjs.revision` records exist — a field whose absent value would have to be *derived from other data* is required, so a revision blob is fully **interpretable in isolation** (no field's meaning depends on fetching another blob). `validate`/`checkReferences` drop the whole snapshot-resolution algorithm (subject-as-fallback, single-parent inheritance, multi-parent rejection) — `subject` is now a pure identity string, never validated as a hash — and instead check `snapshot` is a hash and `generation` is a non-negative integer (layered on the structural schema like `isHash`). `generation`'s correctness is existence + integer-ness only; equality with `1 + max(parents' generations)` is what conforming writers produce, and a deviation is an epoch-reset *signal*, not an invalid blob. The inference moves to the write boundary: `fs/cas/evo`'s `addRevision` now resolves an absent input `snapshot` from the parents (zero → `subject`, one → the parent's snapshot, more than one → error) and computes `generation` (`0` for a root, else `1 + max`), writing every field explicitly. `archived` stays optional — the documented boundary of the rule, since its absent value is the constant `false`. Updates the `fs/media/revision` and `fs/cas/evo` READMEs (schema/field table, generation reframing, interpretable-in-isolation property, future-relaxation-under-new-dialect note) and the `evo_add` tool description; proof coverage for missing/non-integer/negative `generation`, missing/non-hash `snapshot`, a zero-parent non-hash `subject` (now valid), and `add` computing generation across a merge and resolving snapshots. Implements and deletes the `required-fields` issue PR [#1314](https://github.com/functionalscript/functionalscript/pull/1314)
- `fs/types/range`, `fs/bnf`: **BREAKING CHANGE:** `contains` now takes two positional arguments (`contains(a, b)`) instead of a `Range` tuple (`contains([a, b])`); every consumer (`fs/bnf/descent`, `fs/djs/tokenizer`, `fs/text/code_point`, `fs/text/utf16`, `fs/types/range/proof.f.ts`) is updated in the same PR. Building on that, `fs/bnf`'s `eof` sentinel changes from `oneEncode(0x110000)` (one past Unicode's max code point) to `oneEncode(mask)`, the true maximum value representable in the 24-bit `TerminalRange` half — the correct "one past every valid symbol" sentinel for this encoding, distinct from Unicode's own max (`unicodeMax`). `eof`'s packed numeric value is different from before, breaking any code that depended on it directly (e.g. a cached or serialized grammar constant). The now-redundant `max` string constant (`0xFFFFFF`) is removed — its two consumers (`fs/bnf/testlib.f.ts`, `fs/djs/tokenizer/module.f.ts`) import `unicodeMax` instead. `rangeEncode`'s invalid-range check switches from a hand-written `throw` to `assert`. PR [#1308](https://github.com/functionalscript/functionalscript/pull/1308)
- `bnf/descent`: fix `RangeError: Maximum call stack size exceeded` when matching realistic-size input — the matcher `f` recursed natively once per grammar step, so right-recursive rules (how `repeat0Plus` encodes repetition) turned input *length* into JS call-stack *depth*: the DJS tokenizer failed on ~5 KB of short tokens (`' '.repeat(5000)`, reported by Codex review), a ~2,000–3,000-character identifier, a 5 KB string literal, or a 20 KB block comment (a size the previous state-machine tokenizer handled). `f` now runs as an explicit-stack machine — two suspended-frame kinds (sequence: current item index + accumulated ASTs; variant: current branch index + best zero-consumption success) on an immutable cons-cell stack, with a loop that either starts the current rule invocation or feeds the pending result into the innermost frame — with unchanged semantics and O(1) JS call-stack depth for any grammar shape and any input length, for every `descentParser` consumer. This also covers right-recursive shapes that the `repeat`-primitive proposal (`fs/bnf/todo/667-bnf-repeat-flatten.md`) deliberately would not detect, e.g. the tokenizer's hand-written terminated-list `multilineContent` rule — that issue stays open for its original data-shape/flat-AST motivation only, its stack-overflow impact note now historical. `fs/djs/tokenizer` is unchanged apart from factoring the single-token grammar into a local `buildToken()` that `jsGrammar()` wraps with `repeat0Plus` (an interim per-token driver, `tokenizeToAsts`, was landed and then removed within the same PR: with the fixed matcher it was unnecessary, and its per-token `cpm.slice(offset)` made tokenization O(n²) where the whole-file match is linear — ~4× slower at 30,000 tokens). Adds a `longInput` proof group to `fs/bnf/descent` (a 10,000-code-point `repeat0Plus` chain and 5,000 levels of bracket nesting, both verified to throw `RangeError` against the pre-fix matcher restored from git history) and a `largeInputs` group to `fs/djs/tokenizer` (5,000 spaces, 3,000 short id tokens, 5 KB string, 20 KB block comment, 10 KB identifier). The LL(1) backend's matcher recurses the same way and overflows at ~10,000 repetitions (confirmed repro; nothing outside its own proofs consumes it yet) — recorded as `fs/bnf/ll1/todo/stack-recursive-matching.md` with the descent rewrite as the template, including its O(n²) per-step rest-spread to fix alongside. Implements and deletes `fs/djs/tokenizer/todo/stack-recursive-tokenization.md` PR [#1303](https://github.com/functionalscript/functionalscript/pull/1303)
...
- `text/utf8`: dedup the lead-byte classifier in `utf8ByteToCodePointOp` — the fresh-state dispatch and the error-recovery dispatch (after `utf8StateToError`) repeated the same three-way byte check (continuation / valid lead / invalid), including the `0b1100_0010`/`0b1111_0100` lead-byte bounds as unnamed literals in both places. Hoists `leadMin`/`leadMax`/`isLeadByte` next to `contByte`/`contPayload`, and replaces both dispatch copies with a single `restart(prefix)` helper parameterized by the error-code prefix (empty for the fresh-state arm, `[utf8StateToError(state)]` for recovery) — the two arms differ only in that prefix. Pure refactor; `fs/text/utf8/proof.f.ts` passes unchanged. Implements and deletes `lead-byte-classifier-dedup` PR [#1258](https://github.com/functionalscript/functionalscript/pull/1258)
- `fs/`: **BREAKING CHANGE:** regroup top-level directories by concern, the first wave of `fs/todo/group-fs-subdirectories-by-concern.md` — directory paths are FunctionalScript's public API (no `exports` map), so every move below is a breaking change for importers. `base64`, `base128`, and `cbase32` move under a new `fs/basen/` bucket for sibling alphabet-parameterised encoders (`fs/base64` → `fs/basen/base64`, etc.); `fs/types/monoid` moves to a new `fs/common/` bucket for cross-cutting algorithms (`fs/types/monoid` → `fs/common/monoid`); the `fjs` CLI dispatcher is promoted from `fs/fjs/{module.ts,module.f.ts,proof.f.ts,README.md}` to `fs/{module.ts,module.f.ts,proof.f.ts,README.md}` at the package root, updating `package.json`'s `bin.fjs` and scripts and `deno.json`'s `fjs` task accordingly; and `fs/json`/`fs/html` move into a new `fs/media/` bucket for content formats (`fs/json` → `fs/media/json`, `fs/html` → `fs/media/html`). All relative imports and documentation cross-references across the tree are updated to match. `fs/media/revision` (new code), the `fs/mime` → `fs/media/type` rename, and `fs/djs` → `fs/media/djs` are deferred to follow-up PRs per the todo's incremental migration plan PR [#1251](https://github.com/functionalscript/functionalscript/pull/1251)
- `cas/mcp`: **BREAKING CHANGE:** align the `cas_get` response with the MCP resource-contents shape ahead of exposing CAS blobs as `resources/read` resources — `mime_type` is renamed to `mimeType` and `url` to `uri` (the future resource URI for the same blob), and the `content: true` inline payload moves from a single `type`-tagged `content` field to `text` (when `type: 'text'`) or `blob` (when `type: 'base64'`), matching the protocol-defined `resources/read` contents fields. The metadata-only response (`content: false`, the default) keeps `type` as the discriminator for which of `text`/`blob` a later fetch would populate. Clients parsing `cas_get`'s JSON payload must update to the new field names. Updates `fs/cas/mcp/module.f.ts`, `proof.f.ts`, and `README.md` accordingly PR [#1248](https://github.com/functionalscript/functionalscript/pull/1248)
- `nanvm-lib`: fix `BigInt` `Sub` returning the wrong sign when both operands share a sign and `|lhs| < |rhs|` (`3n - 5n` gave `2n` instead of `-2n`) — the `Ordering::Less` arm was mirror-copied from `Add`, where taking `rhs`'s sign is only correct because that arm runs when the signs differ. Fixes the bug by removing the duplication that enabled it: a single `BigInt::add_signed` dispatch parameterized by the effective right-hand sign (`a - b = a + (-b)`), with `Add` passing `rhs`'s sign as-is and `Sub` passing it flipped via the new `Sign::flip` (also used by `Neg` in place of its inline match); the sign is threaded as a parameter rather than negating `rhs`, since `Neg` copies every word of the container. Adds the previously missing signed-dispatch unit tests for `add`, `sub`, and `mul`. Implements and deletes `bigint-add-sub-mirror` PR [#1220](https://github.com/functionalscript/functionalscript/pull/1220)

## 0.36.0

- `cas/mcp`: **BREAKING CHANGE:** remove `type:'url'` from the `cas_add` tool — the server previously opened a filesystem path supplied by the (untrusted) MCP client, guarded only by a `${home}/cas_upload/`-prefix + `..`-rejection string check; a symlink planted inside `cas_upload` (writable by the same caller staging the upload) defeats that check, and no purely-TypeScript fix (leaf `O_NOFOLLOW`, `realpath` containment, or both together) closes every directory-symlink/TOCTOU-race variant — the only airtight fix (per-component `openat()` pinning) isn't reachable without a native addon. `cas_add` now only accepts inline `text`/`base64` content (capped at 128 KiB); large content goes through the `cas` CLI instead (`npx functionalscript cas add <path>`), where the invoking user is the trust boundary, not a sandboxed model. Every remaining server file operation is on a path the server derives itself under `~/.cas/`, documented as a standing design invariant in `fs/cas/mcp/README.md`. Rewrote the large-blob `cas_get` read-path tests to seed the store directly via `c.write` instead of through the removed upload path. Deletes the completed `fs/cas/mcp/todo/remove-local-file-urls-mcp.md` and the two todos that were only kept around pending this removal (`66k-cas-cli-mcp-shared-upload.md`, `66j-normalize-home-paths.md`) PR [#1236](https://github.com/functionalscript/functionalscript/pull/1236)
- `fsc`: use `assertEq` instead of a hand-written `if (result !== x) { throw result }` in the `def` range-map default handler's proof case (added in [#1230](https://github.com/functionalscript/functionalscript/pull/1230)) — an inline `if/throw` in a proof case is itself an uncovered branch (the false side is never exercised in a passing test), so it left `fs/fsc/module.f.ts` short of 100% branch coverage instead of closing the gap; `assertEq`'s own branches are already fully covered elsewhere, so the call site adds none. Documents the pattern as an `AGENTS.md` Testing rule so future proof code reaches for `assert`/`assertEq` by default. PR [#1234](https://github.com/functionalscript/functionalscript/pull/1234)
- `base64`: fix `encode` silently producing a string for an over-`maxLength` vector — padding to the next 6-bit boundary can add up to 4 bits, and the old code built that padding via an explicit `vec`/`concat` before stringifying, neither of which checks `maxLength`, so an `input` within padding distance of `maxLength` (e.g. an exactly-`maxLengthBytes`-sized blob, whose padding needs 2 bits) built an intermediate padded `Vec` past the runtime's `bigint` size ceiling. Fixed by deleting the explicit padding step entirely: `baseN`'s `vecToString` (`fs/base_n/module.f.ts`) already left-pads a trailing partial chunk shorter than its chunk width with zeros, so `encode` can call it directly on `input` — no padded intermediate is ever built, and encoding a `maxLength`-sized vector no longer needs a guard at all (an earlier version of this fix added a `maxLength` check that rejected `null` instead, incorrectly refusing valid boundary input; caught by Codex review). Adds `encodeAtMaxLengthSucceeds` to `fs/base64/proof.f.ts`. Updates `fs/base64/todo/decode-rejects-max-size-input.md` (the paired, still-open decode bug — `encode` no longer being the blocker for its round-trip proof case) and `fs/cas/mcp/proof.f.ts`'s comments to match. Implements and deletes `encode-padding-overflow` PR [#1229](https://github.com/functionalscript/functionalscript/pull/1229)
- `fs/effects`: add `okStep` and `mapOk` step adapters — collapses the hand-written `error → pass through unchanged, ok → continue` check that recurred at five `Effect<O, IoResult<T>>` call sites (`writeLoop`, `writeFromStream`, `fileCas.write`'s `createExclusive` step, `casUpload`, `readUtf8File`), dropping the return-type annotations each site needed only to unify the two branches by hand. `okStep` (`fs/effects/module.f.ts`) is the effectful short-circuit; `mapOk` (`fs/types/result/module.f.ts`) is its pure sibling for mapping just the `ok` case. Documents the step-adapter shape — helpers are continuations passed into `.step`, never wrappers taking the effect — in `fs/effects/module.f.ts`'s module JSDoc, establishing the convention for the effect-helper issues that follow it. Implements and deletes `step-adapters` PR [#1228](https://github.com/functionalscript/functionalscript/pull/1228)
- `js/tokenizer`: reject unescaped control characters in string literals — RFC 8259 §7 requires every U+0000–U+001F code point inside a JSON string to be escaped, but the shared `fs/js` string scanner (`parseStringStateOp`) let a literal control byte through verbatim, so `fs/json`'s `parse` diverged from `JSON.parse` on inputs like `{"a":"⟨TAB⟩"}`. Since `fs/json`, `fs/djs`, and `fs/js` all tokenize strings through this one scanner, and JSON string syntax already covers every JS string value via `\uXXXX`, the fix maps a literal U+0000–U+001F (other than LF/CR, which already end the string) to an `error` token in the single shared grammar — no per-language flag or wrapper-level re-validation needed. Adds proof cases across `fs/js/tokenizer`, `fs/json/parser`, and `fs/djs/parser` covering literal TAB/VT/FF/NUL (rejected) alongside their valid escapes (still accepted). Implements and deletes `reject-unescaped-string-controls` PR [#1218](https://github.com/functionalscript/functionalscript/pull/1218)
- `path`: export `isProperPrefix` from `fs/path/module.f.ts` — moves the pure segment-based containment predicate from its private inline in the virtual FS `rename` handler to the path module where segment-level path semantics belong; adds six proof cases (proper prefix, equal paths, mismatching segment, longer prefix, empty prefix, both empty). Future consumers such as CAS path-boundary hardening can now import it directly from `fs/path`. PR [#1210](https://github.com/functionalscript/functionalscript/pull/1210)
...
- `mcp/stdio`: bound the internal-error fallback added in 0.35.0 — a caller-controlled `id` (e.g. a very large string) can itself push even the `id`-preserving `-32603` fallback past `maxLength`, in which case `writeResponse` now retries once more with a fixed `id: null` internal-error, the only response shape in this transport guaranteed to always encode. Without this second tier that request got no response line at all instead of a fail-soft error. Adds `oversizedIdFallsBackToNullId` to `fs/mcp/stdio/proof.f.ts`. Deletes the now-resolved `fs/cas/mcp/todo/cas-get-response-overflow.md`, moving its design rationale into `writeResponse`'s JSDoc (found by Codex review on PR [#1201](https://github.com/functionalscript/functionalscript/pull/1201)). Also removes the `fs/cas/mcp/proof.f.ts` base64-inflation boundary pair (`getContentBase64InflationOverflowWritesInternalError` / `getContentBase64NearBoundarySucceeds`) added alongside the original fix: `bun test`'s native runner enforces a 5 s per-test timeout, and `base64Encode` + double-JSON-`stringify` on the 90–100 KB `Vec`s those tests needed took 12–14 s under Bun (vs. ~6–7.5 s under Node), failing CI on the `bun` job. The generic transport-level overflow test and the (much cheaper, 33 KB) double-escaping test already cover the same `writeResponse` fallback without constructing bit-vectors anywhere near `maxLength` scale
- `cas/mcp`: `cas_add` now returns a clean `isError` result instead of crashing the server when inline content is malformed or exceeds the 128 KiB limit — switches the `text` branch from the throwing `utf8` to `tryUtf8`, so an over-`maxLength` string yields `null` rather than an unhandled throw on Bun/JSC; collapses both `null` paths (`tryUtf8` and `base64Decode`) into a single static message `'too large or malformed — use type:"url" for large content'` with no content echo; removes the now-unused `utf8` import; adds `base64OfA(n)` proof helper (direct string arithmetic, avoids the encode-padding-overflow bug near `maxLength`); adds four boundary proof cases (one byte over for text and base64, exactly at the limit for text succeeds, exactly at the limit for base64 documents the current `decode-rejects-max-size-input.md` failure); updates `fs/cas/mcp/README.md` and removes the resolved `cas-add-inline-size-error.md` todo PR [#1198](https://github.com/functionalscript/functionalscript/pull/1198)
- `README`: update Getting Started to reflect full `fjs` CLI (`test`, `compile`, `run`, `cas`, `mcp`, `ci` commands); add CAS and MCP Server sections; remove stale NaNVM "Our Next Step"
- `text`: add `tryUtf8`, the `Nullable`-returning sibling of `utf8` — reports `null` instead of throwing when a string's UTF-8 encoding would exceed `maxLength`; `utf8` is now derived from it via `mapUnwrap` instead of duplicating the `fromCodePointList(stringToCodePointList(s))` conversion. Adds round-trip, empty-string, and one-byte-past-`maxLengthBytes` overflow proofs to `fs/text/proof.f.ts` PR [#1196](https://github.com/functionalscript/functionalscript/pull/1196)
- `types/bit_vec` / `base64`: enforce `maxLength` when folding a list into a `Vec` — `tryListToVec`/`listToVec`'s binary-counter accumulator now reports `null`/throws as soon as an in-progress merge would exceed `maxLength`, instead of silently letting `vec()` mask an oversized `bigint`; adds `tryU8ListToVec` as the `Nullable`-returning sibling of `u8ListToVec`. `types/nullable` gains `unwrap` (assert-and-extract) and `mapUnwrap` (lift a `Nullable`-returning function into a throwing one), used to derive each throwing variant from its `try*` counterpart. New overflow proofs cover `tryListToVec`, `listToVec`, `tryU8ListToVec`/`u8ListToVec`, and `base64.decode`, using `assert`/`assertEq` from `fs/asserts` and the test framework's `throw`-key convention. Files three follow-up issues: `fs/base64/todo/encode-padding-overflow.md` (`encode`'s padding can silently push a vector past `maxLength`), `fs/base64/todo/decode-rejects-max-size-input.md` (`decode` wrongly rejects valid input that is exactly `maxLength` after padding-bit trim), and `fs/types/bit_vec/todo/u8-list-to-vec-call-sites.md` (callers that should switch from the throwing to the `try` variant) PR [#1195](https://github.com/functionalscript/functionalscript/pull/1195)
- `types/bit_vec` / `base_n`: make many-into-one bit-vector concatenation O(n log n) instead of O(n²). Extracts the binary-counter accumulator that was private to `u8ListToVec` into a shared `unpackListToVec(unpackConcat)(list)` helper — slot `i` holds an already-combined run of the most recent `2 ** i` elements and each arriving element carries upward, merging only with runs of comparable size, so total `bigint` shifting is O(n log n) with left-to-right order preserved. `BitOrder.listToVec` and `u8ListToVec` now both delegate to it (the duplicated loop is gone), so every many-vector caller (`fs/asn.1`, `fs/crypto/sign`, `fs/sul/...`) gets the fast path. `fs/base_n` `stringToVec` is rewritten to build a chunk list and concatenate once via `msb.listToVec`, dropping the per-character `concat`; it still short-circuits to `null` at the first out-of-alphabet character (O(prefix), never running `normalize` past it). Re-enables the `base_n` `big` proof (1 Mibit hex decode), which drops from ~13 s node / ~43 s bun to well under a second (vec-builder) PR [#1192](https://github.com/functionalscript/functionalscript/pull/1192)
