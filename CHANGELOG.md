...
- `mcp/stdio`: bound the internal-error fallback added in 0.35.0 — a caller-controlled `id` (e.g. a very large string) can itself push even the `id`-preserving `-32603` fallback past `maxLength`, in which case `writeResponse` now retries once more with a fixed `id: null` internal-error, the only response shape in this transport guaranteed to always encode. Without this second tier that request got no response line at all instead of a fail-soft error. Adds `oversizedIdFallsBackToNullId` to `fs/mcp/stdio/proof.f.ts`. Deletes the now-resolved `fs/cas/mcp/todo/cas-get-response-overflow.md`, moving its design rationale into `writeResponse`'s JSDoc (found by Codex review on PR [#1201](https://github.com/functionalscript/functionalscript/pull/1201))

## 0.35.0

- `mcp/stdio`: `writeResponse` now fails soft instead of crashing when an encoded JSON-RPC response line would exceed `maxLength` (128 KiB) — switches from the throwing `utf8` to `tryUtf8`, and on overflow writes a fixed `Internal error` (`-32603`) response carrying the *original request's* `id`, instead of letting the throw escape and kill the MCP server process. Fixes `fs/cas/mcp/todo/cas-get-response-overflow.md`: `cas_get`'s own `length > maxLengthBytes` guard only checks the raw blob size, which can't see base64 inflation (~4/3×) or JSON double-escaping (a literal quote can expand 1 byte into 4 across the inner CAS-JSON pass and the outer JSON-RPC pass) — only the transport's real encode attempt can catch the actual final line size. Removes the now-dead `writeMaybe` helper (folded into `handleLine`, which now threads the original `Response` through so the fallback can read `resp.id`) and the unused `utf8` import. New proof tests: `fs/mcp/stdio/proof.f.ts` `oversizedResponseWritesInternalError` / `loopContinuesAfterOversizedResponse`; `fs/cas/mcp/proof.f.ts` `getContentBase64InflationOverflowWritesInternalError` / `getContentBase64NearBoundarySucceeds` (base64-inflation boundary pair) / `getContentDoubleEscapedOverflowWritesInternalError` (quote-heavy text, uploaded via `type:'url'` to avoid the unrelated cost of parsing a huge inlined JSON-RPC content field) PR [#1201](https://github.com/functionalscript/functionalscript/pull/1201)
