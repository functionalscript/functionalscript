...
- `base64`: fix `encode` silently producing a string for an over-`maxLength` vector — padding to the next 6-bit boundary can add up to 4 bits, and `vec`/`concat` perform no `maxLength` check of their own, so an `input` within padding distance of `maxLength` (e.g. an exactly-`maxLengthBytes`-sized blob, whose padding needs 2 bits) built an intermediate padded `Vec` past the runtime's `bigint` size ceiling and returned a string for a vector `decode` can never read back. `encode` now checks the padded length up front and returns `null` on overflow, consistent with the `Nullable` convention `decode` already uses. Adds `encodeOverflow` to `fs/base64/proof.f.ts`. Updates `fs/cas/mcp/proof.f.ts`'s comments (the `base64OfA` helper already avoided calling `encode` near this boundary for unrelated reasons — computing the expected string directly instead) and `fs/base64/todo/decode-rejects-max-size-input.md` (the paired, still-open decode bug) to reflect the new behavior. Implements and deletes `encode-padding-overflow` PR [#1229](https://github.com/functionalscript/functionalscript/pull/1229)
