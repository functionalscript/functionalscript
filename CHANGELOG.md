...
- `fjs/types/nullable/proof.f.ts` and `fjs/types/range/proof.f.ts` migrate
  to `proof.f.mjs` under the stage-1 TypeScript-to-mjs migration; no other
  module imports a proof file, so this is not a breaking change
