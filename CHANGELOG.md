...
- `text/utf8`: dedup the lead-byte classifier in `utf8ByteToCodePointOp` — the fresh-state dispatch and the error-recovery dispatch (after `utf8StateToError`) repeated the same three-way byte check (continuation / valid lead / invalid), including the `0b1100_0010`/`0b1111_0100` lead-byte bounds as unnamed literals in both places. Hoists `leadMin`/`leadMax`/`isLeadByte` next to `contByte`/`contPayload`, and replaces both dispatch copies with a single `restart(prefix)` helper parameterized by the error-code prefix (empty for the fresh-state arm, `[utf8StateToError(state)]` for recovery) — the two arms differ only in that prefix. Pure refactor; `fs/text/utf8/proof.f.ts` passes unchanged. Implements and deletes `lead-byte-classifier-dedup` PR [#1257](https://github.com/functionalscript/functionalscript/pull/1257)
