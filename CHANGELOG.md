...
- `base64` / `cbase32`: fix `decode`/`cBase32ToVec` wrongly rejecting valid input whose decoded payload is exactly `maxLength`-sized — the `maxLength` cap in `bit_vec`'s `tryListToVec` was checked against the *raw* accumulated length while `stringToVec` was still building the vector, before the caller had a chance to strip the padding bits the encoding added to reach a chunk boundary (2–4 bits for base64's `=` padding, up to 5 for cbase32's sentinel), so an exactly-`maxLength`-sized round trip was rejected. `tryListToVec` (`fs/types/bit_vec/module.f.ts`) and `baseN`'s `stringToVec` (`fs/base_n/module.f.ts`) now take an optional `cap` parameter (defaulting to `maxLength`); `base64.decode` passes `maxLength + removeBits` (the exact padding this input declares) and `cbase32.cBase32ToVec` passes `maxLength + 5n` (the sentinel's worst case), then re-validate the trimmed length as before. Adds `maxLengthRoundtrip` to `fs/base64/proof.f.ts` and `fs/cbase32/proof.f.ts`. Flips `fs/cas/mcp/proof.f.ts`'s `addBase64AtLimitIsError` to `addBase64AtLimitSucceeds`, since a maximally-sized (128 KiB) base64 `cas_add` now stores cleanly instead of erroring. Implements and deletes `fs/base64/todo/decode-rejects-max-size-input.md`
