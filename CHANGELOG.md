...
- `fs/media/revision`: add the `vnd.fjs.revision` content format — a CAS BLOB representing one step in the evolution of a mutable object, linking to its parent revision(s) (a DAG, so concurrent edits can merge) and carrying the full materialized snapshot; incremental diffs are deliberately excluded (a future `vnd.fjs.change` dialect instead, per the versioning rule: additive changes keep the `dialect` tag, breaking changes get a new one). `fs/media/revision/module.f.ts` defines the RTTI schema, the `dialect` tag, and `validateRevision`/`decodeRevision`/`encodeRevision`; `parents`/`snapshot`/the zero-parent fallback `subject` are validated as cbase32 hashes (rejecting `https://` bridge URLs), with the snapshot-resolution cardinality rules (fallback subject / single-parent inheritance / multi-parent-requires-snapshot) layered on top of RTTI's structural check. `fs/media/type`'s `detectVec` now recognizes a buffered revision blob by its `{"dialect":"vnd.fjs.` prefix and full schema validation, reporting `application/vnd.fjs.revision+json`; the unbounded `detectStream` path is untouched. Store-touching evolution operations (head resolution, materialization) and `cas_get` response shaping are deferred to a follow-up. Implements the first phase of `fs/cas/todo/revision-content-format.md` PR [#1263](https://github.com/functionalscript/functionalscript/pull/1263)
