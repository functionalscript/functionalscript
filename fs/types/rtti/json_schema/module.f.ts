 ...
 *
 * Mirrors the visitor structure of `../ts/module.f.ts` (`printer` / `toTs`),
 * but emits a JSON Schema object instead of a TypeScript type string.
 *
 * @module
 */
import type { Const, Type } from '../module.f.ts'
import { or } from '../module.f.ts'
...
const admitsUndefined = (rtti: Type): boolean => {
    ...
    const info = rtti()
    if (info[0] !== 'or') { return false }
    return (info.slice(1) as readonly Type[]).some(admitsUndefined)
}
...
const stripUndefined = (rtti: Type): Type => {
    ...
    const info = rtti()
    if (info[0] !== 'or') { return rtti }
    const rest = (info.slice(1) as Type[]).filter((t): t is Type => t !== undefined)
