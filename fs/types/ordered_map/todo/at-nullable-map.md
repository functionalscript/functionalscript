...
`map` in `fs/types/nullable/module.f.ts` (`f => value => value === null ?
null : f(value)`) — and `array`'s safe accessors already route through it.
`ordered_map.at` (`fs/types/ordered_map/module.f.ts:23-28`) re-inlines the
same shape by hand:
...
- `fs/types/nullable/module.f.ts` — the combinator.
- `fs/types/array/module.f.ts` — precedent: safe accessors already use it.
