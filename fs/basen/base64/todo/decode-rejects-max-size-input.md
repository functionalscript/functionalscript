...
unaudited: `fs/basen/cbase32/module.f.ts`'s `cBase32ToVec` builds its decoded `Vec`
...
- [ ] Confirm whether `cBase32ToVec` actually rejects a legitimate
      ...
      `fs/basen/base64/proof.f.ts` for cbase32, using `fs/basen/cbase32/proof.f.ts`).
...
`base64.decode` (`fs/basen/base64/module.f.ts`) now decodes every body character
