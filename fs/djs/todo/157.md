...
`fs/media/json/parser/module.f.ts` and `fs/djs/parser/module.f.ts` implement the
...
- `fs/media/json/module.f.ts:49` (`serialize`)
...
`fs/media/json/serializer/module.f.ts`, so the leaves are shared; only the walker was
copied.
...
> Stale as of `fs/djs/tokenizer/todo/replace-old-tokenizer.md`: the old
> state-machine `fs/djs/tokenizer` this item describes was deleted and
> replaced by a grammar-based tokenizer (formerly `fs/djs/tokenizer-new`,
> renamed to `fs/djs/tokenizer`). The same minus-folding pattern was ported
> into that file's `mapDjsToken`/`parseDjsDefaultState`/`parseDjsMinusState`/
> `scanDjsToken`, so the dedup opportunity with `fs/media/json/tokenizer`
> likely still exists, but the line numbers and function names below are
> outdated — re-verify before acting on this.

`fs/media/json/tokenizer/module.f.ts:27–101` and the old
`fs/djs/tokenizer/module.f.ts:26–116` (since replaced) both wrapped
...
> `fs/djs/tokenizer-new/module.f.ts`, `todo()`-stubbed) was a separate
