...
In `fs/djs/ast/module.f.ts` the two container branches of `toDjs` are
...
- [ ] `npx tsc` clean; `fjs t` passes (`fs/djs/ast/proof.f.ts` and the djs
      suite).

### Related

- `fs/djs/ast/module.f.ts:29-32`, `:41-46`, `:48-70` — the types and code
  involved.
