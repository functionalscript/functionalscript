...
Both `fs/media/json/parser/module.f.ts` and `fs/djs/parser/module.f.ts` build the
...
#### JSON (`fs/media/json/parser/module.f.ts:79-111`)
...
- [ ] In `fs/media/json/parser/module.f.ts`, add `pushStack` / `popState` (or
...
- [ ] Apply the same shape to `fs/djs/parser/module.f.ts`, preserving the
      ...
      `build` callback.
- [ ] Run `npx tsc` and `fjs t`; confirm `fs/media/json/parser/proof.f.ts` and
