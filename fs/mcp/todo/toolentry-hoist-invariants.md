...
Two factories in `fs/mcp/module.f.ts` re-derive call-invariant values inside
per-call closures, against the AGENTS.md rule *"Hoist call-invariant
computations out of function bodies"*:

1. `toolEntry` (`fs/mcp/module.f.ts:179-194`) rebuilds the validator on
...
2. `fromRegistry` (`fs/mcp/module.f.ts:216-233`) rebuilds the full `tools`
...
`fs/types/rtti/parse/module.f.ts:164`. The casts stay (with a comment citing
...
- [ ] Add a comment on the remaining casts citing TS2589, mirroring
      `fs/types/rtti/parse/module.f.ts:164`.
- [ ] `npx tsc` clean; `fjs t` passes (`fs/mcp/proof.f.ts`, `fs/cas/mcp/proof.f.ts`).

### Related

- `fs/mcp/module.f.ts:179-194`, `:216-233` — the two factories.
- AGENTS.md — "Hoist call-invariant computations out of function bodies";
  curried-application placement rule.
- `fs/types/rtti/parse/module.f.ts:164` — precedent for the TS2589 cast
  comment.
