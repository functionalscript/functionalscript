...
`fs/common/monoid/module.f.ts`. That module ships `repeat`, which consumes
...
- [ ] Add `fold` to `fs/common/monoid/module.f.ts` with proof coverage.
...
- `fs/common/monoid/module.f.ts` — `repeat`, the existing `Monoid` consumer.
