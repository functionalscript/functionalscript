...
export const proof = {
    ...
    decodeRoundTrip: () => {
        ...
        assert(v === vec(8n)(0xabn), [v])
    },
    decodeInvalid: () => {
        assert(hex.stringToVec('z') === null, 'invalid char should return null')
        assert(hex.stringToVec('aZ') === null, 'mixed invalid char should return null')
    },
    normalizeHit: () => {
        ...
        assert(cb32.stringToVec('I') === cb32.stringToVec('1'), 'I→1')
        assert(cb32.stringToVec('l') === cb32.stringToVec('1'), 'l→1')
        assert(cb32.stringToVec('o') === cb32.stringToVec('0'), 'o→0')
    },
    normalizeMiss: () => {
        assert(cb32.stringToVec('u') === null, 'unknown char should return null')
    },
