...
The same public type is declared in two modules. `fs/bnf/module.f.ts:23`:

```ts
export type TerminalRange = number
```

and `fs/bnf/data/module.f.ts:25-27`:
...
Define `TerminalRange` once in `fs/bnf/module.f.ts` (the module that owns the
range encode/decode primitives) and have `fs/bnf/data/module.f.ts` import and
...
- `fs/bnf/todo/669-bnf-data-shared-helpers.md` — other `bnf/data` DRY cleanups
