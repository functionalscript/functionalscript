...
`fileCas.read`'s inner `loop` (`fs/cas/module.f.ts:140-153`):
...
`streamFile`'s inner `loop` (`fs/cas/module.f.ts:251-259`):
...
`fs/effects/list/module.f.ts`.) The only real difference is the declared effect
...
**Caveat on the type.** `casAddFile` (`fs/cas/module.f.ts:267-271`) is *not* a
...
- `fs/cas/module.f.ts:267-271` — `casAddFile`, which absorbs `ReadBytes` via a
