...
- Per [remove-local-file-urls-mcp](../mcp/todo/remove-local-file-urls-mcp.md)
  the MCP server no longer opens local
...
- [remove-local-file-urls-mcp](../mcp/todo/remove-local-file-urls-mcp.md) —
  removes MCP local-path upload, making filesystem ingestion (including
  directories) a CLI-only concern
