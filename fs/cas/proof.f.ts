...
import { foldStepWhile, pure, pureList, type Effect, type ListEffect, type ListStep } from '../effects/module.f.ts'
...
// Drains an (unbounded) CAS read stream into an array of chunk `Vec`s. Stops at the first
// empty `ok` chunk (EOF) or surfaces an `error` item as the result.
const drainVecs = foldStepWhile(
    (item: IoResult<Vec>) => (acc: readonly Vec[]): Effect<never, ListStep<readonly Vec[], IoResult<readonly Vec[]>>> =>
        item[0] === 'error'
            ? pure(['stop', item])
            : length(item[1]) === 0n
                ? pure(['stop', ok(acc)])
                : pure(['next', [...acc, item[1]]]),
    (acc: readonly Vec[]): Effect<never, IoResult<readonly Vec[]>> => pure(ok(acc)),
)([])
...
export const proof = {
    ...
    casWriteRead: () => {
        ...
        const [, readResult] = virtual(state1)(drainVecs(c.read(hash)))
    ...
    casReadMissingShard: () => {
        ...
        // draining the read stream stops with an `error` result, not an empty `ok([])`.
        const c = fileCas(sha256)('.')
        const hash = computeSync(sha256)([vec8(0x2An)])
        const [, result] = virtual(emptyState)(drainVecs(c.read(hash)))
        if (result[0] !== 'error') { throw ['missing shard must surface as an error item', result] }
    },
    casWriteMultiChunk: () => {
    ...
    casWriteReadExceedsMaxLength: () => {
        ...
        const rehash = foldStepWhile(
            (item: IoResult<Vec>) => (state: typeof sha256.init): Effect<never, ListStep<typeof sha256.init, IoResult<Vec>>> =>
                item[0] === 'error'
                    ? pure(['stop', item])
                    : length(item[1]) === 0n
                        ? pure(['stop', ok(sha256.end(state))])
                        : pure(['next', sha256.append(item[1])(state)]),
            (state: typeof sha256.init): Effect<never, IoResult<Vec>> => pure(ok(sha256.end(state))),
        )(sha256.init)
        const [, readBack] = virtual(state1)(rehash(c.read(hash)))
    ...
    casWriteGcReclaimsExpired: () => {
        ...
        const content = vec8(0x2An)
        const c = fileCas(sha256)('.')
        const x = c.write(pureList([ok(content)]))
