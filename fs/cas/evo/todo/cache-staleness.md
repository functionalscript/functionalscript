...
The Evo cache (`fs/cas/evo/module.f.ts`) is built once at server startup
...
see `fs/cas/mcp`). Nothing else refreshes it.
...
[`fs/cas/evo/README.md`](../README.md)'s "In-memory cache is per process"
...
- [ ] Update `fs/cas/evo/README.md` and `fs/cas/evo/mcp/README.md` to
      describe the staleness/consistency guarantees once decided.

### Related

- [`fs/cas/evo/README.md`](../README.md) — "In-memory cache is per process":
  the adjacent (but distinct) multi-process cache-sharing problem.
- `fs/cas/todo/66j-cas-periodic-stage-recovery.md` — an existing precedent
  for periodic/best-effort background maintenance in this same store.
- `fs/cas/todo/web-api-server.md` — a shared HTTP(S) server would have one
