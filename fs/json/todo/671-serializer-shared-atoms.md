...
shares with JSON from `fs/json/serializer/module.f.ts` (line 15:
...
In `fs/json/serializer/module.f.ts`, next to `objectWrap`/`arrayWrap`:
...
`fs/json/module.f.ts` imports `colon` and instantiates
...
- [ ] Export `colon` and `MapEntries<P>` from `fs/json/serializer`.
- [ ] Delete the local copies in `fs/json/module.f.ts` and
