...
```ts
// fs/json/rpc/module.f.ts:87 — private
const errorResponseOf = (id: Id) => (error: RpcError): Response =>
    ({ jsonrpc, error, id })
// fs/json/rpc/module.f.ts:116 — success shape inlined in dispatch
    ? { jsonrpc, result, id }
...
Export both constructors from `fs/json/rpc/module.f.ts` and consume them in
`dispatch` and `mcp`:

```ts
// fs/json/rpc/module.f.ts
...
- [ ] Export `errorResponseOf`; add and export `successResponseOf` in
      `fs/json/rpc/module.f.ts`; use it in `dispatch`.
- [ ] Replace `_errResponse`/`_okResponse` in `fs/mcp/module.f.ts` with the
      imported constructors.
- [ ] `npx tsc` clean; `fjs t` passes (`fs/json/rpc/proof.f.ts`,
      `fs/mcp/proof.f.ts`).

### Related

- `fs/json/rpc/module.f.ts` — owner of the `Response` envelope.
