...
/**
 * Effect-based TTY-aware writer. The `isTTY` flag is read from
 * `NodeProgramOptions.std` (known at process startup, never changes). When the
 * target stream is not a TTY, ANSI SGR escape sequences are stripped before
 * writing.
 */
export const csiWrite =
    (o: NodeProgramOptions) =>
