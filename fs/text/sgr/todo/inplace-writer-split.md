...
`fs/text/sgr/module.f.ts` bundles two unrelated concerns. Alongside the ANSI
...
consumer (only `fs/text/sgr/proof.f.ts:17` calls `createConsoleText`), which
...
(e.g. `fs/text/console/module.f.ts`), leaving `sgr` focused on
