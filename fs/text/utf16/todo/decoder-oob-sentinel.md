...
a small helper next to `decoder` in `fs/text/code_point/module.f.ts`:

```ts
export const outOfRange = <S>(state: S): readonly [readonly number[], S] => [[errorMask], state]
```

Check `fs/text/utf16/proof.f.ts` for assertions pinning `0xffffffff` and
