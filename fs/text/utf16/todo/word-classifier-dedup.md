...
`utf16ByteToCodePointOp` (`fs/text/utf16/module.f.ts:188-207`) contains the
...
`restart(prefix)` helper (`fs/text/utf8/module.f.ts:83-92`); utf16 is the
...
Since utf8 already has its own `restart` (`fs/text/utf8/module.f.ts:88-92`),
...
`fs/text/code_point/module.f.ts` (next to `decoder`; utf8 imports from
...
- [ ] Add the generic `restart` to `fs/text/code_point/module.f.ts` (with
