...
`fs/text/code_point/module.f.ts` owns the surrogate/BMP boundary as named
...
predicates — re-hardcodes the same numbers in `fs/text/utf16/module.f.ts`:
...
```ts
// fs/text/code_point/module.f.ts
...
`codePointToUtf16` (`fs/text/utf16/module.f.ts:87-96`) and
...
- [ ] Add `toSurrogatePair`/`fromSurrogatePair` (and `supplementaryBase`) to
      `fs/text/code_point/module.f.ts`; cover them in its proof.
- [ ] Rewrite the two utf16 sites through them.
- [ ] `npx tsc` clean; `fjs t` passes (code_point/utf16 proofs).

### Related

- `fs/text/code_point/module.f.ts:50-54` — the constants and their
...
- `fs/text/utf8/todo/error-tag-layout-constants.md` — the analogous
