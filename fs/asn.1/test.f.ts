import { empty, length, msb, msbConcat, uint, unpack, vec, vec8, type Vec } from "../types/bit_vec/module.f.ts"
...
export default {
    ...
    encodeDecode: {
        integer: () => {
            ch([integer, 0n], msbConcat([vec8(BigInt(integer)), vec8(1n), vec8(0n)]))
            ch([integer, 1n], msbConcat([vec8(BigInt(integer)), vec8(1n), vec8(1n)]))
        },
        sequence: () => {
            ch([constructedSequence, []], msbConcat([vec8(BigInt(constructedSequence)), vec8(0n)]))
            ch(
                [constructedSequence, [[integer, 0n]]],
                msbConcat([vec8(BigInt(constructedSequence)), vec8(3n), encode([integer, 0n])])
            )
            ch(
    ...
    raw: [
        () => {
            ...
            if (e !== msbConcat([vec8(0x00n), vec8(1n), vec8(0x23n)])) { throw `encode: ${length(e)}: ${uint(e).toString(2)}` }
        ...
        () => {
            ...
            if (e !== msbConcat([vec8(0x1Fn), vec8(0x20n), vec8(2n), vec(16n)(0x1234n)])) {
                const l = length(e)
                const u = uint(e)
