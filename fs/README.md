...
```sh
node ./fs/module.ts <command> [args]
```
...
- `fs/module.f.ts` itself, which uses `export const main`.
...
```
fs/module.ts          — Node.js entry point (runs main via the node runner)
fs/module.f.ts        — FunctionalScript command dispatcher (Commands list + dispatch)
