...
`fs/media/json` exposes its pure lexer and parser as two separate modules —
`tokenize` in `fs/media/json/tokenizer/module.f.ts` and
`parse: (tokenList) => Result<Unknown, string>` in
`fs/media/json/parser/module.f.ts` — but offers **no** single "parse a string
...
Add a Result-returning string parser to `fs/media/json/module.f.ts` built on
...
- [ ] Add `parseText` to `fs/media/json/module.f.ts` with proof coverage.
