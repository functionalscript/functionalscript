...
A module `fs/media/json/html/module.f.ts` that converts a JSON `Unknown` value to a
...
- [ ] `fs/media/json/html/module.f.ts` — `toHtml(value: Unknown): VDom` converter
- [ ] `proof.f.ts` covering each value type and a nested example
- [ ] Register in `deno.json` exports

### Related

- `fs/media/json/module.f.ts` — the `Unknown` type and `serialize`
- `fs/media/json/schema/module.f.ts` — sibling JSON-dialect module
