...
// const resultMap = (v, f) => 'error' in v ? v : f(v.value)
...
const error = ev => (_, e) => e(ev)

/** @type {(id: ErrorId) => (...params: string[]) => ErrorResult} */
const errorId = id => (...params) => error({ id, params })

/** @type {(_: string) => Result<ModuleId>} */
const parseRepo = repo => {
    const s = repo.split('/')
    return s.length === 2 ? value({ repo: { org: s[0], name: s[1] } }) : errorId('expected')('/', repo)
}

/** @type {(repo: string, branch: string) => Result<ModuleId>} */
const parseRepoAndBranchCommit = (repo, branch) => parseRepo(repo)(module => value({ repo: module.repo, branch }), error)
...
const parseProtocolAndId = (protocol, id) => protocol === 'github'
    ? parseGitHubId(id) : errorId('unknownProtocol')(protocol)

module.exports = {
    parseModuleUrl: splitOne(':', parseProtocolAndId, errorId('expected')),
    /** @type {ErrorMap} */
    errorMap: {
