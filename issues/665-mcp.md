...
### 1. JSON-RPC 2.0 layer — [i665-json-rpc](./665-json-rpc.md)

The envelope: request / notification / response / error schemas, decoders, and the
pure dispatcher. MCP rides directly on this. **Prerequisite.**
...
### 3. rtti → JSON Schema printer — [i665-rtti-json-schema](./665-rtti-json-schema.md)
...
*once* in rtti and expose it over MCP, we need a sibling printer
`fs/types/rtti/json_schema/` (`toJsonSchema`) that maps an rtti `Type` to a JSON
Schema object — analogous to `toTs` but emitting `{ type, properties, required,
items, … }`. This is the main capability MCP needs that JSON-RPC does not.
...
- **`id` / number handling.** Inherit the decision from i665-json-rpc.

## Suggested decomposition (future issues)

1. rtti → JSON Schema printer (`toJsonSchema`) — [i665-rtti-json-schema](./665-rtti-json-schema.md).
2. MCP core schemas + lifecycle (initialize, capabilities, ping, tools).
3. stdio transport over `fs/effects/node`.
4. resources / prompts / logging schemas.
5. Streamable HTTP transport.
6. bidirectional (sampling / roots) support.

## Related

- [i665-json-rpc](./665-json-rpc.md) — the JSON-RPC 2.0 envelope (prerequisite)
