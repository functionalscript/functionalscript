...
The simplest example, for synchronizing from a trusted source: if a CAS is stored as files, synchronization can be nothing more than copying one CAS on top of another *without overwriting existing files*. We call this **synchronization by copying files**. No destruction is possible — files that already exist are never overwritten, and the receiver simply gains the blobs it was missing. Rehashing is still what ultimately preserves the `same hash = same content` invariant — each blob must hash to the address it sits under — but for a trusted source this check does not have to block the copy: we can copy first and run hash verification later, deleting any blob that fails to match its address. A corrupted, truncated, or misnamed file is thus caught and removed lazily rather than rejected up front. Until a copied blob has been verified — either by the deferred scrub or by a verifying read — it is only *provisional*: it occupies its address but is not yet proven content, and any reader that requires authenticity must verify it (rehash and compare) before using it rather than trusting the bytes solely because they sit under the expected path. Deferring the check trades a window of provisional blobs for a cheaper copy; it never lets unverified bytes be treated as authentic content. The same logic generalizes to any transport: HTTP, peer-to-peer gossip, a USB drive, email attachments. The protocol only determines how blobs travel; the correctness of the merge is guaranteed by content addressing alone.
