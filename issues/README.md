...
- [ ] [201-bun-inline-context](./201-bun-inline-context.md). Bun's `node:test` throws `ERR_NOT_IMPLEMENTED` on `t.test()` inside callbacks; eager flat registration is impossible because leaves are only discoverable after executing `fn`. Fix: add `inlineContext` to `fs/types/effects/node/module.f.ts` (runs sub-tests inline), and use `bunTestContext` in `fs/io/module.ts` to pass `inlineContext` to thunks instead of `t`.
