...
- [ ] [181-csi-edsl](./181-csi-edsl.md). Introduce a structured eDSL for composing ANSI CSI/SGR sequences in `fs/text/sgr/module.f.ts`, analogous to the HTML eDSL in `fs/html/module.f.ts`. A `Block = readonly [Options, readonly (string | Block)[]]` tree replaces hand-rolled template-literal concatenation.
