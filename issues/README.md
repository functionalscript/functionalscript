...
- [x] 130. Optimization of `or` in [../types/rtti/module.f.ts](../types/rtti/module.f.ts). Analyze the variants of an `or` schema and group similar items together (e.g. objects with overlapping shapes, primitive consts) so dispatch can be faster than a linear scan over every variant.
  ...
  Done: primitive const variants are now grouped into a `Set` at construction time (via `analyzeOr`, attached to the thunk via a module-scoped `WeakMap` and queryable via `orAnalysis`). `validate` and `parse` reuse the precomputed analysis for O(1) primitive matching; objects/tuples/thunks still scan linearly. Manually constructed `or` thunks fall back to computing the analysis on dispatch.
