...
- [ ] [187-byte-rounding-divup](./187-byte-rounding-divup.md). DRY + type-honesty: `asn.1` re-derives byte-rounding (`(x + 7n) >> 3n`, `byteLen << 3n`) that `bigint.divUp`/`roundUp` already provide (also used by `crypto/sign`). Reuse `divUp(8n)`/`roundUp(8n)`, and retype `divUp`/`roundUp` from `Reduce` to `(divisor: bigint) => Unary` since the first arg is config, not a fold accumulator.
