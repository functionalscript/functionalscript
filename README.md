...
- [JSON](https://en.wikipedia.org/wiki/JSON) and [JSON5](https://json5.org/) as subsets of JavaScript values.
  JSON is also a subset of FunctionalScript values.
...
`undefined`), and property access on a name — `a.b`, `a[0]` — which only the
EDAG output compiles so far. Functions and other computed expressions are not
supported yet. See
