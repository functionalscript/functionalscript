...
const frontTest = (e: BitOrder) => (r0: bigint) => (r1: bigint) => () => {
    ...
    if (vector !== unsafeVec(0x1F5n)) { throw vector }
...
const removeFront = (e: BitOrder) => (r0: Vec) => (r1: Vec) => () => {
    ...
    if (v !== unsafeVec(0x13456n)) { throw v }
...
const concat = (e: BitOrder) => (r: Vec) => () => {
    ...
    if (a !== unsafeVec(0x145n)) { throw a }
    const b = u8(0x89n) // 0x189n
    if (b !== unsafeVec(0x189n)) { throw b }
...
export default {
    examples: {
        vec: () => {
            ...
            if (v0 !== unsafeVec(0x15n)) { throw v0 }
            const v1 = vec4(0x5FEn) // 0x1En
            if (v1 !== unsafeVec(0x1En)) { throw v1 }
        },
        uint: () => {
            const vector = vec(8n)(0x5n) // 0x105n
            if (vector !== unsafeVec(0x105n)) { throw vector }
        ...
        removeFront: () => {
            ...
            assertEq(lsb.removeFront(4n)(v), asNominal(0x1345n))
            assertEq(lsb.removeFront(24n)(v), asNominal(0x1n))

            assertEq(msb.removeFront(4n)(v), asNominal(0x1456n))
            assertEq(msb.removeFront(24n)(v), asNominal(0x1n))
        },
        popFront: () => {
            const vector = vec(8n)(0xF5n) // 0x1F5n

            assertEq2(lsb.popFront(4n)(vector), [5n, asNominal(0x1Fn)])
            assertEq2(lsb.popFront(16n)(vector), [0xF5n, asNominal(1n)])

            assertEq2(msb.popFront(4n)(vector), [0xFn, asNominal(0x15n)])
            assertEq2(msb.popFront(16n)(vector), [0xF500n, asNominal(1n)])
        },
        concat: () => {
            const u8 = vec(8n)
            const a = u8(0x45n) // 0x145n
            ...
            assertEq(lsb.concat(a)(b), asNominal(0x18945n))
            assertEq(msb.concat(a)(b), asNominal(0x14589n))
        }
    },
    front: {
    ...
    removeFront: {
        lsbm: removeFront(lsb)(asNominal(0x1345n))(asNominal(0x1n)),
        msbm: removeFront(msb)(asNominal(0x1456n))(asNominal(0x1n)),
    },
    concat: {
        lsbm: concat(lsb)(asNominal(0x18945n)),
        msbm: concat(msb)(asNominal(0x14589n)),
    },
    uintLsb: () => {
        const vector: Vec = asNominal(0b1110101n)
        const extract3Bits = lsb.front(3n)
        const result = extract3Bits(vector) // result is 0b101n (5n)
        if (result !== 0b101n) { throw result }
    },
    uintSmall: () => {
        const vector: Vec = asNominal(0b11n)
    ...
    vecExample: () => {
        ...
        if (vector !== unsafeVec(0b10101n)) { throw vector }
    },
    length: () => {
