    ...
    or: {
        ...
        unknownVariant: {
            ok: () => {
                ...
                assertOk(validate(t)(42))
                assertOk(validate(t)('hello'))
                assertOk(validate(t)(null))
                assertOk(validate(t)({}))
            },
        },
        manyTypes: {
            ok: () => {
                // Exercises every typeof bucket in one schema.
                const t = or(boolean, number, string, bigint, undefined, null, array(number))
                assertOk(validate(t)(true))
                assertOk(validate(t)(42))
                assertOk(validate(t)('hi'))
                assertOk(validate(t)(7n))
                assertOk(validate(t)(undefined))
                assertOk(validate(t)(null))
                assertOk(validate(t)([1, 2]))
            },
            error: () => {
                const t = or(boolean, number, string, bigint, undefined, null)
                // Object that does not match `null` falls through every bucket.
                assertError(validate(t)({}))
                assertError(validate(t)([1]))
            },
        },
        objectShapes: {
            ok: () => {
                // Both variants share `typeof === 'object'`; both must remain reachable.
                const t = or(array(number), record(string))
                assertOk(validate(t)([1, 2, 3]))
                assertOk(validate(t)({ a: 'x' }))
            },
            error: () => {
                const t = or(array(number), record(string))
        ...
        sameBucket: {
            laterMatch: () => {
                ...
                assertOk(validate(t)(2))
                assertOk(validate(t)(3))
            },
            allFail: () => {
                const t = or(...[1, 2, 3] as const)
                assertError(validate(t)(4))
            },
        },
        // A struct/tuple `Const` used directly as an `or` variant — typeof of
        // the schema itself is `'object'`, and the bucket is `'object'`.
        constObject: {
            ok: () => {
                const t = or([42] as const, string)
                assertOk(validate(t)([42]))
