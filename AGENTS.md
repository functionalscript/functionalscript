...
and ends its description with a matching `Changelog:` section; a PR that doesn't
— internal refactors, test-only changes, and PRs that only touch `todo/`,
`AGENTS.md`, or other documentation — needs neither. Breaking changes are
welcome when they improve the API — prefix the entry with
`**BREAKING CHANGES:**` and update every importer in the same PR.
