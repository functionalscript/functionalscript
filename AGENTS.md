...
- When a sibling module already has the type or helper you need, import it — add `export` to the existing declaration if it's not yet exported, rather than duplicating it (e.g. `parse` reuses `Path`, `ValidationError`, `verror`, `prependPath`, `primitive0Validate`, `constPrimitiveValidate` from `validate`). The same applies to canonical constants and standard built-ins: use the existing `primitive0List` instead of inlining `['boolean', 'number', 'string', 'bigint']`, and reach for built-ins like `Object.is` directly instead of wrapping them in a hand-rolled helper.
