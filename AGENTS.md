...
- Principles:
  ...
  - Separation of concerns — move logic to its natural module even with a single consumer when the logic is conceptually distinct and the target module already exists (e.g. path manipulation belongs in `fs/path`, not inline in a loader). This is different from DRY extraction: it is always appropriate.
