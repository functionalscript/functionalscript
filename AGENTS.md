...
- Issues are tracked in `./issues/`, not on GitHub. To file a new issue: create `./issues/{N}-{slug}.md` (next sequential number, short kebab-case slug) and add a `- [ ] [N-slug](./N-slug.md). One-line summary.` entry at the bottom of the list in `./issues/README.md`. Do not open GitHub issues.
