...
What a new fixture needs is two edits nobody lists — a `fjs compile` into
`gen.fixtures/` in `package.json`'s `gen`, and a `pub mod` line in
`src/lib.rs`'s `fixtures` module, which one `#[path]` points at that
directory — and two fixtures showed the cost: `function` and `rest-function`
were committed "@generated by `fjs compile`", but neither was compiled by `gen`
nor included by `src/lib.rs`, so they were neither drift-checked nor built.
