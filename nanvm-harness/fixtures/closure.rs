...
pub fn module<A: IStaticFunction>() -> Result<Any<A>, Any<A>> {
    let c0: Any<A> = A::static_function(|_self, args| { Ok(A::static_function(|self_, args| {
        ...
        c1 + c2
    }, 0, [args.clone().to_any()].to_array()).to_any()) }, 0, Array::default()).to_any();
    ...
    let c9: Any<A> = A::static_function(|_self, args| { Ok(A::static_function(|self_, args| {
        ...
        Ok(A::static_function(|self_, args| {
            ...
            let c5: Any<A> = Any::dot(args.clone().to_any(), f64_any(0x0000000000000000)).end()?;
            let c6: Any<A> = Any::dot(c1.clone(), f64_any(0x0000000000000000)).end()?;
            Ok([c2, c4, c5, c6].to_array().to_any())
        }, 0, [c0, args.clone().to_any()].to_array()).to_any())
    }, 0, [args.clone().to_any()].to_array()).to_any()) }, 0, Array::default()).to_any();
