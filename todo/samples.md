...
- The root [README.md](../README.md) explains what FunctionalScript *is*
  ...
  and documents CAS and MCP usage. Its "Compiling a module" section shows two
  small modules and what `fjs compile` writes for them, sharing included — but
  that is compiler input, not a program a newcomer writes and runs. Beyond it,
...
scratch: its `math.f.js` example is the seed of the first sample. Its
shared-reference example already has a counterpart in the root README's
"Compiling a module" section, where one imported value is referenced twice and
stays shared in the output. Once the rest lives in `samples/`, delete it.
...
- [fjs/website/todo/generate-website.md](../fjs/website/todo/generate-website.md)
  — the `demo` export convention and README→HTML publishing; samples should
