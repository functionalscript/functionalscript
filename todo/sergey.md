...
- [ ] BNF
  ...
  - [ ] Workflow: create BNF rules, create rule maps, then create a parser from
        a root rule, a default `Meta` monoid and the map rules. The parser is a
        state machine over one input symbol at a time, with a validatable root
        output. Design and open questions:
