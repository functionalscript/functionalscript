...
Synchronizing from a trusted source needs no *provenance* check — we already trust the signer, so there is no trust filter to apply — but it still needs the same hash verification as any other source: a trusted peer can still hand us a blob that disk corruption or truncation has made inconsistent with its address, and that blob must not be served as authentic content (the protocol-agnostic section below describes how this verification can be deferred safely). What "trusted" removes is the social filtering, not the integrity check. Synchronizing from an *untrusted* source is also safe at the transport level — content addressing makes the merge harmless regardless of where blobs come from — but we add a trust filter on acceptance: we keep only content whose provenance we can verify against our web of trust. Provenance is itself CAS — a separate signature block `{ content_hash, signature, signer_pubkey }` from a trusted DID — not an inline signature on the content blob, so acceptance verifies a content blob *together with* its signature/provenance blocks rather than requiring each blob to be individually signed. Content and its signature blocks may arrive in any order or over different transports; a blob is retained once some trusted signature block referencing its hash has also been synced. The source can offer anything, but everything that cannot be tied back to a trusted signer is discarded.
...
The simplest example, for synchronizing from a trusted source: if a CAS is stored as files, synchronization can be nothing more than copying one CAS on top of another *without overwriting existing files*. We call this **synchronization by copying files**. No destruction is possible — files that already exist are never overwritten, and the receiver simply gains the blobs it was missing. Rehashing is still what ultimately preserves the `same hash = same content` invariant — each blob must hash to the address it sits under. For a trusted source this check does not have to block the copy, but deferring it is only safe if the unverified blobs are kept out of the authenticated read path until they pass: either copy into a quarantine/staging namespace and promote a blob into the live store only after its hash matches, or serve every read through verification (rehash and compare on the way out). A corrupted, truncated, or misnamed file is then caught — and deleted or never promoted — before any reader can mistake it for authentic content. Copying unverified blobs straight into the live tree and reading them back without rehashing does **not** provide this guarantee: a reader between the copy and the deferred scrub would consume the bad bytes under a hash they do not produce. Today's plain `cas get` is exactly such a non-verifying read, so the deferral depends on one of those two mechanisms actually being in place (tracked in [i66G-cas-verify-command](../../fjs/cas/todo/66g-cas-verify-command.md) and [i66G-cas-get-verify-option](../../fjs/cas/todo/66g-cas-get-verify-option.md)). The same logic generalizes to any transport: HTTP, peer-to-peer gossip, a USB drive, email attachments. The protocol only determines how blobs travel; the correctness of the merge is guaranteed by content addressing alone.
