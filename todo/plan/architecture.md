...
Planned refactor: extract `casMcpStep` from `casMcpServer` in `fjs/protocol/mcp/cas/module.f.ts`. Currently `casMcpServer` wires stdio directly; once extracted, the stdio server becomes a one-liner and the HTTP server is additive, not a rewrite.
