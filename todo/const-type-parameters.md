...
**Priority:** P2
**Status:** implemented — 8 modifiers added, 68 of 268 `@type {const}` casts removed
...
2. **Every modifier must be load-bearing.** Each added `const` was removed again
   in isolation to confirm `npx tsc` then fails. Of the 15 tried, 12 changed
...
`npx tsc` and `fjs t` (3127 proofs) pass.
...
68 casts went with them, the densest clusters being the two rtti reader proofs
...
`validate`, `parse`, and `result` proofs pin the inference, so dropping a
modifier fails the build rather than silently widening a schema.
...
The 200 remaining casts are not this rule's to remove:

| Shape | Count | Why the modifier cannot reach it |
| --- | --: | --- |
| Declaration pin — `const x = /** @type {const} */ (…)` | 120 | the literal widens at the declaration, before any call; the "Pin literal `const`s" rule governs, and a later `validate(x)` only sees what `x` already is |
...
- **`protocol/mcp` `toolEntry`, `media` `dialectEntry`, `types/array`
  `includes`.** Every caller passes an already-pinned named schema. Worth
  revisiting the day one inlines a literal.
...
- A **checked** pin for the 120 declaration sites: `export const x = type({…})`
