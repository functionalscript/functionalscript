...
[harness export selection](../nanvm-harness/todo/select-module-export.md) gives
the consumer the same: `nanvm-harness::run(module, export, action)` reads or
calls one named export. The acceptance example below does not run through it
yet. Neither feature requires named function parameters.
...
both JavaScript EDAG evaluators. The harness's `run` now provides that
selection and call; moving this test onto it is the rest of the
export-selection task, which also says why the harness has no CLI. Source
round trips cover the serializer's admitted expressions; calls and arithmetic are still refused by
...
- [ ] Implement [harness export selection](../nanvm-harness/todo/select-module-export.md)
      ...
      The API is implemented; running the acceptance example through it
      remains.
