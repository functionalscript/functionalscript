...
/**
 * @template T
 * @typedef {(update: (value: T) => Result<T>) => Result<T>} Filter
 */

/** @type {<T>(cmp: Cmp<T>) => (filter: Filter<T>) => (node: N<T>) => Result<T>} */
const apply = cmp => filter => {
    ...
    const f = node => {
        switch (node.length) {
            case 1: {
                const [v] = node
                switch (cmp(v)) {
                    case -1: return filter(value => ({ replace: [value, v] }))
                    case 1: return filter(value => ({ replace: [v, value] }))
                }
                return { done: v }
            }
            case 2: {
                const [v0, v1] = node
                switch (cmp(v0)) {
                    case -1: return filter(value => ({ overflow: [[value], v0, [v1]] }))
                    case 0: return { done: v0 }
                }
                switch (cmp(v1)) {
                    case -1: return filter(value => ({ overflow: [[v0], value, [v1]] }))
                    case 0: return { done: v1 }
                    default: return filter(value => ({ overflow: [[v0], v1, [value]] }))
                }
            }
            case 3: {
                const [n0, v0, n1] = node
...
const insert = cmp => value => apply(cmp)(f => f(value))

/** @type {<T>(cmp: Cmp<T>) => (node: N<T>) => Result<T>} */
const search = cmp => apply(cmp)(() => ({ done: undefined }))
