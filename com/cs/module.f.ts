...
const unsafe = (isUnsafe: boolean): string => isUnsafe ? 'unsafe ' : ''

const fullType = (t: Type): readonly [boolean, string] =>
    typeof (t) === 'string' ?
        [false, baseType(t)] :
    t.length === 1 ?
        [false, t[0]] :
...
const type = (t: Type): string => fullType(t)[1]

const param = ([name, t]: Field): string => `${type(t)} ${name}`

const mapParam = map(param)

const field = ([name, comType]: Field): string => {
    const [isUnsafe, t] = fullType(comType)
    return `public ${unsafe(isUnsafe)}${t} ${name};`
}

const isUnsafeField = (field: Field): boolean =>
    fullType(field[1])[0]
...
const method = ([name, m]: O.Entry<FieldArray>): readonly string[] => {
...
const def = ([n, d]: O.Entry<Definition>): List<Item> =>
    !('interface' in d) ?
 ...
 */
export const cs = (name: string) => (library: Library): Block => {
