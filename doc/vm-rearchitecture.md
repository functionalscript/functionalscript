...
In Nov 2023, the FunctionalScript team started a new project called NaNVM. We have limited resources (⌛💰) for the projects, so progress has been slow. Since then, we’ve implemented several components from scratch in Rust:
- An interface and multiple implementations for [Memory Management](../nanvm-lib/src/mem), such as a [global](../nanvm-lib/src/mem/global.rs) memory manager using standard `alloc/dealloc`, a [local](../nanvm-lib/src/mem/local.rs) manager with a reference counter, and a simple [arena](../nanvm-lib/src/mem/arena.rs) implementation.
- [All FS data types](../nanvm-lib/src/js) in the VM, such as [string](../nanvm-lib/src/js/js_string.rs), [bigint](../nanvm-lib/src/js/js_bigint.rs), [array](../nanvm-lib/src/js/js_array.rs), [object](../nanvm-lib/src/js/js_object.rs), and [any](../nanvm-lib/src/js/any.rs).
