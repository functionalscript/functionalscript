...
`accessed` in
[`fjs/fsc/parser/reader`](./fsc/parser/reader/module.f.mjs) are the
ordinary shape, beside the exported `_Frame` and `_NegFrame` in the
parser's [`private.ts`](./fsc/parser/private.ts), which are the rule's. Measured across `fjs/`, module-private constants run about 1,900
