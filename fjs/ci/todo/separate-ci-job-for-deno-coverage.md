...
- [ ] Add a `deno-coverage` job to the CI generator (`fjs/ci/module.f.ts` or a new `fjs/ci/deno/module.f.mjs` variant) that runs `deno task cov` (runs `deno test --allow-read --allow-env && deno coverage --include='.*module\\.f\\.ts'`, matching the `npm run cov` scope).
