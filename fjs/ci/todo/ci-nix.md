## ci-nix. Generate exact-version Nix environments

**Priority:** P3
**Status:** open
**Phase:** 2
**Depends on:** Phase 1 — exact versions in `fjs/ci/config/module.f.ts` (done)
**Unlocks:** [direct Nix CI](ci-nix-ci.md), [Nix input locks](ci-nix-locks.md), and [Nix hardening](ci-nix-hardening.md)
...
[Phase 7](ci-nix-hardening.md). Their absence does not block Phase 3.
...
- converting CI jobs to Nix — [Phase 3](ci-nix-ci.md);
- Linux OCI images — [Phase 4](ci-scenario-docker.md);
- macOS cache investigation — [Phase 5](ci-nix-cache-macos.md);
- committed Nix input locks — [Phase 6](ci-nix-locks.md);
- comprehensive validation and fixed artifact hashes —
  [Phase 7](ci-nix-hardening.md);
- a developer-oriented aggregate `flake.nix`.

### Related

- [rollout overview](dockerfile-nix-integration.md) — complete dependency-driven
  rollout.
