...
shapes it does not handle. Each was declined there; this file is where the
answers live now.
...
**3. A package that ships `.ts` sources and no declarations.** `include` is
...
- **(3)** Either enumerate declaration extensions in `include` — the
...
- [ ] Build (3) the day root `package.json` `files` admits a `.ts` source, and
      not before.
