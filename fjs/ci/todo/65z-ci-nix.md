...
The CI generator in `fjs/ci/` would emit the `shell:` override or the
...
- [ ] Decide whether the CI generator (`fjs/ci/`) emits `shell:` overrides or
      explicit `nix develop --command` prefixes
- [ ] Determine if tool versions in `fjs/ci/config/module.f.ts` can be
