...
- [ ] In `fjs/ci/module.f.ts`, read the repo root for `deno.lock` and `bun.lock` (via `access`) and pass the results to the job builder, analogous to how `Cargo.toml` controls Rust steps.
...
- [ ] Update `fjs/ci/proof.f.ts` to cover the new conditional logic.
