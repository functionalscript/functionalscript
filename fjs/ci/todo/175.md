...
- [i170](./170.md) — the `toolSteps` step-sequence builder; this factory feeds
  it. This entry used to read `i170/i171`, but `i171` is not a CI issue: it was
  `tf: stop relying on JS function names to detect throw tests`, resolved
  **won't fix** with the reason recorded in `parseTestSet`'s JSDoc in
