...
3. Run `npm run lock-update` to propagate everything.
...
- [ ] Wire `fjs u` (or equivalent) into the `lock-update` script so dependency bumps are automated again.
