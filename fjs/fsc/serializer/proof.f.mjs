 ...
 * each key, a function body under a rest parameter and under two named
 * ones, a node shared twice, each side of a root comma, a root comma whose
 * two sides are one node, a root comma with one operand, the same graph
 * twice over in two scopes, and a function that captures the graph beside
 * it.
 *
...
export const proof = {
    ...
    // A function of count `0` is written with its rest parameter and no
    // other, since a body reads its arguments as one node: the parameters
    // are named as a spreadsheet names its columns, by depth, so a body
    // names its own and reaches the ones outside it.
    functions: () => {
    ...
    // A function of count `n` is written with `n` named parameters, the
    // body's letters, `_` and the position — `$a_0` beside the hoist name
    // `$a0` — one bare for a count of one and a list otherwise, unused
    // ones included, since the count is what `length` reads as. A
    // parameter is the read of its position, `['.', ['args'], i]`, and the
    // one spelling the arguments have under names: the array itself, a
    // position past the count and a key that is no position are refused,
    // since `[$a_0, $a_1]` is not the complete arguments the node denotes.
    parameters: () => {
        /** @type {(i: number) => Exp} */
        const arg = i => ['.', ['args'], i]
        writes(['=>', 1, null, arg(0)], 'export default $a_0=>$a_0;')
        writes(['=>', 2, null, ['[]', [arg(1), arg(0)]]], 'export default ($a_0,$a_1)=>[$a_1,$a_0];')
        writes(['=>', 3, null, arg(1)], 'export default ($a_0,$a_1,$a_2)=>$a_1;')
        writes(['=>', 2, null, 1], 'export default ($a_0,$a_1)=>1;')
        // a parameter is a base as any name is, and `-0` reads position
        // `0` — the same program, read back with the key's sign lost, as a
        // frame slot's is
        writes(['=>', 1, null, ['.', arg(0), 'length']], 'export default $a_0=>$a_0.length;')
        assertEq(unwrap(tryStringify(['=>', 1, null, ['.', ['args'], -0]])), 'export default $a_0=>$a_0;')
        ...
        /** @type {Exp} */
        const o = ['{}', []]
        writes(['=>', 1, null, ['[]', [o, o]]], 'export default $a_0=>{const $a0={};return [$a0,$a0];};')
        writes(['=>', 1, null, ['=>', 2, null, arg(1)]], 'export default $a_0=>($b_0,$b_1)=>$b_1;')
        writes(['=>', 2, null, ['=>', 0, null, ['args']]], 'export default ($a_0,$a_1)=>(...$b)=>$b;')
        // a parameter captured by a nested function is a name already, as
        // a slot is, so it takes no `const` — where a rest parameter's
        // position does, being no name
        writes(['=>', 1, null, ['=>', 1, ['[]', [arg(0)]], ['[]', [['.', ['frame'], 0], arg(0)]]]], 'export default $a_0=>$b_0=>[$a_0,$b_0];')
        writes(['=>', 0, null, ['=>', 1, ['[]', [arg(0)]], ['[]', [['.', ['frame'], 0], arg(0)]]]], 'export default (...$a)=>{const $a0=$a[0];return $b_0=>[$a0,$b_0];};')
        writes(
            ['=>', 2, null, ['=>', 0, ['[]', [arg(0), arg(1)]], ['=>', 1, ['[]', [['.', ['frame'], 0], ['args'], ['.', ['frame'], 1]]], ['[]', [['.', ['frame'], 0], ['.', ['frame'], 1], arg(0), ['.', ['frame'], 2]]]]]],
            'export default ($a_0,$a_1)=>(...$b)=>{const $b0=$b;return $c_0=>[$a_0,$b0,$c_0,$a_1];};')
        // what the compiler builds is written and read back, the three
        // spellings each
        assertEq(reads(_defaultExport(moduleGraph('export default a => [a, a];'))), 'export default $a_0=>[$a_0,$a_0];')
        assertEq(reads(_defaultExport(moduleGraph('export default (a) => a;'))), 'export default $a_0=>$a_0;')
        assertEq(reads(_defaultExport(moduleGraph('const c = [1]; export default (a, b) => { const x = [a, c]; return (d) => [x, b, d]; };'))),
            'const $0=[1];export default ($a_0,$a_1)=>{const $a0=[$a_0,$0];return $b_0=>[$a0,$a_1,$b_0];};')
        // the boundary: the arguments under names have no name of their
        // own, and a read that is no position has none either
        refuses(['=>', 1, null, ['args']], 'the arguments of a function with named parameters')
        refuses(['=>', 2, null, ['[]', [arg(0), ['args']]]], 'the arguments of a function with named parameters')
        refuses(['=>', 2, null, ['.', ['args'], 'length']], 'an argument that is no named parameter')
        refuses(['=>', 2, null, arg(2)], 'an argument that is no named parameter')
    ...
    //
    // The graphs are every shape over every shape over the atoms: 3070 of
    // them, of which 1703 have a text and the rest are refused by name.
    ...
    // of the rest, which is what a body's `const`s are worth to it. The
    // shape of two named parameters is 340 of the graphs and 256 of the
    // texts, the rest refused at the arguments a name does not spell.
    law: () => {
        generated.forEach(e => {
