...
`-1` is the negation of `1` in the parser's tree; the lowering folds that one
case back into the leaf, so every reader of the graph sees the number it saw
before there was an operator, and [`rust`](rust/module.f.mjs) prints it as it
always did. A negation of anything else stays a node, and that route refuses
one, `Neg for Any<A>` answering a `Result` a module cannot hold. It binds looser than a
