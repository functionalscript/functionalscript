...
export const proof = {
    ...
    func: {
        ...
        // A call on a numeric literal is taken, though an access on one is
        // refused. The tokenizer folds a minus into the number, so `-1()`
        // here calls `-1` where JavaScript reads `-(1())` and calls `1` —
        // but a number is callable under neither reading, so nothing a
        // program can observe divides them, and the call is the EDAG its
        // operands make. An access divides them by value, which is why that
        // one is refused: `-1 .x` is `undefined` here and `NaN` there.
        numericCallee: () => {
            ...
            // an argument of a numeric callee is read as any other is, so
            // its own error is the one reported
            const [tag, value] = parseFromTokens(tokenizeString('export default 1(zzz);'))
            assert(tag === 'error', tag)
            assertEq(value.message, 'const not found')
            assertEq(value.metadata?.column, 18)
            // and a reference to a number reads alike in both, as before
