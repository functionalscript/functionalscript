/**
 * Type-level API for `fjs/fsc/parser/reader/module.f.mjs`: the nodes its
 * rewrite set builds and the alphabet it returns them in — the input
 * alphabet is the grammar's, in `../grammar/types.ts`, and the error a
 * read reports is the parser's `ParseError`, in `../types.ts`.
 *
 * @module
 */
