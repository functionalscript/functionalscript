...
reader keeps a token's text and judges none of it: it takes a property
key's and a module specifier's word as spelled, and the two helpers it
exports for the fold, `_nameOf` and `_textOf`, are how a word reaches the
one module that decides whether it may be bound, imported or read —
