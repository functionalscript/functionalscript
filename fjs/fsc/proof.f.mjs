...
export const proof = {
    ...
    fjsOutput: {
        ...
        graph: () => {
            ...
            // parameter's function does, the AST carrying a count of `0`
            // and no name, so the writer gives it the one spelling it has
            // — which denotes the same function, a name being unobservable
            // and the count the same
            assertEq(fjsRoundTrip('export default () => 1;'), 'export default (...$a)=>1;')
            // named parameters are written by count and position: the
            // writer's names, as many as the count, in the body's letters
            assertEq(fjsRoundTrip('export default a => [a, a];'), 'export default $a_0=>[$a_0,$a_0];')
            assertEq(fjsRoundTrip('export default (a, b) => [b, a];'), 'export default ($a_0,$a_1)=>[$a_1,$a_0];')
            assertEq(fjsRoundTrip('export default (a, b) => a;'), 'export default ($a_0,$a_1)=>$a_0;')
        },
