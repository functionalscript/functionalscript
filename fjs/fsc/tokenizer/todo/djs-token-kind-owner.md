...
- `fjs/fsc/tokenizer/types.ts:27-39` — the `DjsToken` union itself;
- `fjs/fsc/tokenizer/module.f.mjs:635-661` — `mapDjsToken`'s 23-label
  `switch (input.kind)` that narrows `JsToken` to `DjsToken`;
- `fjs/fsc/parser/grammar/module.f.mjs` — `_tokenKindNames`, the same set
  minus `eof`.
