...
export const operations = {
    ...
    // the graph's, and `length` is the record's field — the declared count,
    // which JavaScript reads off a function as it reads an array's, while
    // the arguments are the complete list under any count. A count the
    // language does not declare — `1.5`, `-1`, `NaN`, `-0` — is refused
    // rather than carried as a `length` no JavaScript function has: the
    // schema checks the number's shape only, and the executor is where the
    // graph is last seen before it runs.
    '=>': ({ operand }) => ([, count, frameExp, body]) => {
        assert(isCount(count), ['=>: count is no nonnegative integer', count])
        return /**@type {Closure<typeof body>}*/({ mark, length: count, frame: operand(frameExp), body })
    },
    '>': order((a, b) => a > b),
    '>=': order((a, b) => a >= b),
    '>>': o2((a, b) => a >> b),
