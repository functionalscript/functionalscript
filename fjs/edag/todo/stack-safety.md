...
overflows the JS call stack. At `36c8d4a`, an 8,000-term `1 + 1 + … + 1`
compiled to `.rs` fails in `dispatch`:

```sh
$ fjs compile in.f.js out.rs
...
there rather than reaching that refusal. That refusal is gone — every operator prints
