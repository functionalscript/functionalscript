...
export const proof = {
    ...
    // The count is evaluated in the enclosing scope, as the frame is.
    length: () => {
        assertEq(callable(run(['=>', 2, null, 5])).length, 2)
        /** @type {Exp} */
        const n = ['.', ['args'], 0]
        const f = callable(run(['=>', 0, null, ['[]', [['=>', n, null, 1], n]]]))
        const [g, m] = array(f(4))
        assertEq(callable(g).length, 4)
        assertEq(m, 4)
    },
