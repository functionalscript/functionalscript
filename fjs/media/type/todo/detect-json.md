...
`cas_get` (`fjs/protocol/mcp/cas/module.f.ts:204`) picks up `application/json`
...
next to the signature table (`:18-31`) and in `fjs/protocol/mcp/cas/module.f.ts`'s
...
- [ ] Update `fjs/media/type/module.f.ts` module doc (recognised-types table) and the
      `cas_get` output section in `fjs/protocol/mcp/cas/module.f.ts` to list
      `application/json`.
...
- `fjs/protocol/mcp/cas/module.f.ts:196-204` — `cas_get`, the consumer that gains `application/json` for free.
