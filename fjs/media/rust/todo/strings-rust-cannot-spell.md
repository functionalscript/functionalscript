...
- [ ] Escape control characters with `\u{…}`; prove U+0000, U+001F and
      U+007F round-trip through a generated module in `nanvm-harness`.
- [ ] Return a `Result` from `stringLiteral` and `i64Literal`; move the two
      throws to refusals the printers carry; prove `fjs compile` reports a
      lone surrogate and an out-of-range bigint as diagnostics.
