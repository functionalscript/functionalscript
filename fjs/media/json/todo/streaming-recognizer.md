...
1. **The parser builds the whole value.** `parse`
   (`fjs/media/json/parser/module.f.mjs:233-240`) accumulates objects/arrays in
...
- **Value-free parsing.** Drive `fjs/media/json/parser`'s per-token control machine
  (`foldOp` — `fjs/media/json/parser/module.f.mjs:206-223`) with a no-op value builder,
...
- `fjs/media/json/parser/module.f.mjs:206-240` — `foldOp` / `parse`; the control machine to reuse value-free.
