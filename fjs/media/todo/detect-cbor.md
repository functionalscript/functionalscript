...
The `fjs/media/type` detector knows nothing about CBOR. A CBOR
...
- [ ] Add the tag-55799 prefix (`0xD9 0xD9 0xF7`) to the `fjs/media/type` magic table →
...
- [ ] Surface the derived type in `cas_get` / resource read alongside the JSON
      ...
      [fjs/protocol/mcp/cas cas-get-mcp-resource-response](../../protocol/mcp/cas/todo/cas-get-mcp-resource-response.md)
