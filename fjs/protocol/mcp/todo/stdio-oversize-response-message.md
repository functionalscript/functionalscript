...
`evo_revision` (`fjs/protocol/mcp/cas/evo`) inherit the same envelope.
...
- [ ] Check the `cas_get` overflow proofs in `fjs/protocol/mcp/cas/proof.f.ts`, which
...
- `fjs/protocol/mcp/cas/evo/README.md` — the "Result size" note describing this
