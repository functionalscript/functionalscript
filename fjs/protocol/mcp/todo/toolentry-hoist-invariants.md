...
Two factories in `fjs/protocol/mcp/module.f.mjs` re-derive call-invariant values inside
per-call closures, against the AGENTS.md rule *"Hoist call-invariant
computations out of function bodies"*:

1. `toolEntry` (`fjs/protocol/mcp/module.f.mjs:158-168`) rebuilds the validator on
...
2. `fromRegistry` (`fjs/protocol/mcp/module.f.mjs:201-215`) rebuilds the full `tools`
