...
- [157](./157-json-djs-shared-value-machine.md) — its §2 shares one *serializer
  walker* between the two families, and depends on this: JSON's `treeSerialize`
  is already typed over `Tree<P>`, so DJS has to name that shape before a walker
  can be shared rather than duplicated. Its parser sub-task, by contrast, is
  superseded — the DJS state machine it would have shared no longer exists.
