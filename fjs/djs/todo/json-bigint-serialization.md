...
Every JSON number in `fjs/media/json`'s pipeline collapses to a JS `number`,
even though the tokenizer computes an exact value first and then throws the
precision away: `NumberToken.bf: BigFloat` (`fjs/js/tokenizer/module.f.ts:81-85`)
is exact, but `fjs/media/json/parser/module.f.ts:121`'s `tokenToValue`
...
types are depended on by `fjs/protocol/mcp`, `fjs/protocol/mcp/cas`, `fjs/protocol/mcp/stdio`, `fjs/fsm`,
...
`fjs/djs` (a JS-superset format transpiled from/to `.f.ts`) already has the
right leaf type: `Primitive = JsonPrimitive | bigint | undefined`
(`fjs/djs/module.f.ts:22`). But its only bigint encoding today is the JS
literal suffix — `fjs/types/bigint/module.f.ts:90`: `` `${a}n` `` — which is
not valid JSON. This is not just a theoretical gap: `fjs/djs/module.f.ts`'s
`compile` **already has a live bug** from it. Its `.json`-output branch —

```ts
// fjs/djs/module.f.ts:42-44
...
(`fjs/djs/serializer/module.f.ts:133-135`), whose leaf switch has
`case 'bigint': { return [bigintSerialize(value)] }`
(`fjs/djs/serializer/module.f.ts:120`) — emitting `123n`. So compiling any
djs module that contains a bigint to an output path ending in `.json`
**writes invalid JSON to a `.json` file**, silently. `fjs/djs/proof.f.ts`'s
`jsonOutput` test only exercises `export default 42` (a plain number), so
this is untested and unnoticed today.

### Proposal

Add a second parse/serialize pair to `fjs/djs` — alongside the existing
...
- **Parse.** Reuse `fjs/media/json/tokenizer` as-is (it already rejects
  ...
  to `number` as today. Don't duplicate `fjs/media/json/parser/module.f.ts`'s
...
- **Serialize.** Reuse `fjs/media/json/serializer`'s leaf-agnostic atoms
  ...
  `fjs/djs/serializer/module.f.ts:15`). Add a `bigint` case emitting plain
  ...
  `e`.
- **Fix the live bug.** Once this exists, `fjs/djs/module.f.ts`'s `compile`
...
- **Naming and placement.** Candidates: `parseJson`/`stringifyJson` (reads
  ...
  directly on `fjs/djs/module.f.ts`, or a new `fjs/djs/json_bigint/module.f.ts`
...
- [ ] Add `parseWith` (name TBD) to `fjs/media/json/parser/module.f.ts`,
...
- [ ] Settle the value-type open question above (plain `djs.Unknown` vs. a
      ...
      type is chosen.
- [ ] Implement the new parse function in `fjs/djs` using
      `fjs/media/json/tokenizer` + `parseWith` with a `bigint`-for-bare-integers
      `tokenToValue`.
- [ ] Implement the new serialize function reusing
      `fjs/media/json/serializer`'s atoms, with a plain-digit `bigint` case
      and `.0`-forcing on whole `number`s.
- [ ] Fix `fjs/djs/module.f.ts`'s `compile` `.json` branch to use the new
...
- [ ] Document the convention (`3` = bigint, `3.0` = float) and its
      relationship to djs's native `123n` encoding, in `fjs/djs/README.md`.
- [ ] `npx tsc`, `fjs t`.
...
- [todo/new-pl.md § Numbers](../../../todo/new-pl.md#numbers) — the
  ...
  inside today's `fjs/djs` rather than a change to literal syntax.
...
- `fjs/djs/proof.f.ts`'s `jsonOutput` test — the existing (too narrow)
