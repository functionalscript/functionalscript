...
`fjs/rtti/common/module.f.mjs:visit` was built for: it parametrises
...
- **Don't preemptively unify with `rtti/common/visit`.** RTTI's `visit`
  ...
  RTTI side, and should wait for a third real consumer of the *unified*
  abstraction. The RTTI side reaching three consumers of its own is not
  that: `rtti/common/visit` walks `Type`, this walks `Unknown`.
...
- i172 (retired; shipped as [`fjs/rtti/validate/`](../../rtti/validate/module.f.mjs)
  and [`fjs/rtti/parse/`](../../rtti/parse/module.f.mjs)) — a similar
...
- i143 (retired; shipped as [`fjs/rtti/data/`](../../rtti/data/module.f.mjs)) —
  ...
  from [`fjs/rtti/common/`](../../rtti/common/module.f.mjs)
  ...
  side, whose reasons to defer are unrelated to that count and unchanged: the
  `pre`-hook the const short-circuit needs still has one consumer
  (`serializeWithConst`) against a bar of two, and i157's serializer factoring
  should land first.
