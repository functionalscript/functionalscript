...
- `has` (`byte_set/module.f.mjs:15`, `((s >> BigInt(n)) & 1n) === 1n`) has no
  existing `bigint` equivalent; leave it as is — this issue is only about the
  `mask` duplication.
- Unrelated to the dead `nibble_set` of [i160](../nibble_set/module.f.mjs) —
  resolved won't fix, the decision recorded in that module's header.
