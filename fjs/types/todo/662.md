...
`fjs/types/rtti/common/module.f.ts` exports `visit` — a visitor over the
...
```ts
// fjs/types/rtti/common/module.f.ts:99
...
`fjs/types/rtti/ts/module.f.ts` — and it never adopted `visit`. It
re-hand-rolls the identical recognition logic:

```ts
// fjs/types/rtti/ts/module.f.ts:118
...
(`fjs/types/ts/module.f.ts:23-29`), with `union` and `primitive` from the
same module (`:44`, `:55`).
...
- [rtti data form](../rtti/todo/data-form.md) — the prospective fourth `Type`-ADT consumer
  (the serializable data form); a printer already on `visit` is one fewer
  fork it has to reconcile with.

- `fjs/types/rtti/common/module.f.ts:99,124` — `visitConst` / `visit`, the
  shared schema walker.
- `fjs/types/rtti/ts/module.f.ts:118-145` — the printer's duplicated
  `constToTs` + `toTs` switch.
- `fjs/types/ts/module.f.ts:23-29,44,55` — the printer leaves the visitor
