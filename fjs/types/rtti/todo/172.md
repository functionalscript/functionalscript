...
`fjs/types/rtti/validate` and `fjs/types/rtti/parse` each expose a factory pair
...
container walk exists — the serializable data form
([`../data`](../data/README.md)) has since landed and consumes `eachEntry`,
so that bar is met. Revisit, designing the skeleton as a
...
- [`../data`](../data/README.md) — the RTTI data form; the third consumer.
