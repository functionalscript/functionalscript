...
`fjs/types/rtti/common/module.f.mjs` exports `visit` — a visitor over the
...
```ts
// fjs/types/rtti/common/module.f.mjs:164
...
```ts
// fjs/types/rtti/common/module.f.mjs:79
...
`fjs/types/rtti/ts/module.f.mjs` — and it never adopted `visit`. It
re-hand-rolls the identical recognition logic:

```ts
// fjs/types/rtti/ts/module.f.mjs:41
...
```ts
import { visit } from '../common/module.f.mjs'
import type { Visitor } from '../common/types.ts'
...
(`fjs/types/ts/module.f.mjs:23-29`), with `union` and `primitive` from the
same module (`:44`, `:55`).
...
- **Import direction / cycle.** `common` already does
  `/** @import { Ts } from '../ts/types.ts' */` (type-only). Adding
  `import { visit } from '../common/module.f.mjs'` to `ts` makes the two
  modules co-dependent at compile time. At **runtime** the edge is
  one-directional (`ts → common`; the `common → ts` import is type-only
...
- [`../data`](../data/README.md) — the fourth `Type`-ADT consumer
  (the serializable data form); a printer already on `visit` is one fewer
  fork it has to reconcile with.

- `fjs/types/rtti/common/module.f.mjs:79,164` — `visitConst` / `visit`, the
  shared schema walker.
- `fjs/types/rtti/ts/module.f.mjs:41-64` — the printer's duplicated
  `constToTs` + `toTs` switch.
- `fjs/types/ts/module.f.mjs:23-29,44,55` — the printer leaves the visitor
