...
`fjs/types/bit_vec/module.f.ts` defines a fully generic accumulator type and
...
(`fjs/types/bit_vec/module.f.ts:311-315` and `:363-374`.) All bit-vector
...
short-circuit. That is `fjs/types/list` territory — the module that already
...
Move the type and driver into `fjs/types/list/module.f.ts`:
...
- [ ] Add `Accumulator` and `tryFold` to `fjs/types/list/module.f.ts` with
...
- `fjs/types/list/module.f.ts` — `fold`, `foldScan`, `stateScan`, the
