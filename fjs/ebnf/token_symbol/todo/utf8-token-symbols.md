...
[`../README.md`](../README.md) already weighed this
...
[the DJS parser](../../../fsc/parser/README.md), the consumer this was written for, builds its
...
This depends on [bigint-symbols](../../terminal/todo/bigint-symbols.md), which
is on hold for the same reason, so reviving this means reviving both.

Revive when a token symbol has to be written to a file.

### Problem

`fjs/ebnf/token_symbol` assigns each token name the symbol at its position
in a registered alphabet, past `0x110000`. This works for small token sets, but it
...
Replace the registration-based `fjs/ebnf/token_symbol` mapping with a
deterministic mapping from a token name to the 256-bit `Symbol` introduced by
[bigint-symbols](../../terminal/todo/bigint-symbols.md).
...
[the DJS parser](../../../fsc/parser/README.md) consumes `token_symbol.encoding()` deliberately and
...
- [ ] Migrate the token-name alphabets of dependent parser designs, including
      [the DJS parser](../../../fsc/parser/README.md), onto this fallible `Symbol` mapping as part
      of reviving this task. Do not block them on it in the meantime.
- [ ] Replace callers of `fjs/ebnf/token_symbol` with the UTF-8 mapping and handle
      the nullable result explicitly.
- [ ] Remove `fjs/ebnf/token_symbol` only after all callers and dependent designs
      have migrated.
...
- [256-bit bigint symbols](../../terminal/todo/bigint-symbols.md) — provides
  the full uint256 ordinary-symbol space used by this mapping; EOF remains `-1`.
- [`fjs/ebnf/README.md`](../../README.md#terminals-and-eof) — EOF is outside
  the physical symbol domain.
- [The DJS parser](../../../fsc/parser/README.md) — uses `token_symbol.encoding()` today; the
  caller this task would migrate, not a task waiting on it.
- [Layered parser](../../todo/layered-parser.md) — tokenizer output feeds the
  next parser as one symbol per token plus metadata.
- [`fjs/ebnf/token_symbol`](../README.md) — registration-based mapping
