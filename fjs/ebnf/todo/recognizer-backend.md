...
**Status:** blocked
**Blocked by:**
- [EOF as an ordinary input symbol](../terminal/todo/eof-as-ordinary-symbol.md) — the
...
The two other blockers it had are gone: the byte helpers a recognizer over
bytes consumes shipped as [`../byte`](../byte/README.md), and the data
`Rule` union carries every repetition bound natively
([`../data`](../data/README.md)), so a builder discriminates one settled
union.
...
This TODO previously also assigned ownership of binary authoring helpers to
the recognizer work. Those are [`../byte`](../byte/README.md)'s, shipped;
the recognizer consumes them rather than creating a second byte-helper API
or restoring alphabet-specific syntax in the front end.

The recognizer must also preserve the logical EOF contract the backend
ships ([`../README.md`](../README.md#logical-eof-in-parser-input)): `EOF = -1`,
...
[eof-as-ordinary-symbol](../terminal/todo/eof-as-ordinary-symbol.md) lands first, callers
...
[eof-as-ordinary-symbol](../terminal/todo/eof-as-ordinary-symbol.md) replaces.** If EOF becomes
...
```
functional grammar (fjs/ebnf)  ──lower──▶  data RuleSet (fjs/ebnf/data)  ──build──▶  automata
   (composable authoring)                   (serializable IR)
```

Which front end, and which lowering, is not the builders' business: a
hand-written or deserialized rule set is as good an input as a lowered one.

The data IR is exactly the substrate for automaton construction:
alternation, concatenation, a range-set terminal and a bounded repetition,
with name references for recursion — the tagged tuples
[`../data`](../data/README.md) defines — and a builder compiles a bounded
...
not here. `parserRuleSet` in [`../ll1`](../ll1/README.md) is already *one*
family built from `RuleSet`; the recognizer and DFA backends are **new
builders over the same `RuleSet`**, its siblings — not a separate front end.
So: author `magic | utf8` in the front end, lower it, compile; the builders
never see where the rule set came from.

Module layout follows from this: the data module holds only the
serializable IR and the lowering, and each parser/automaton builder lives
in its own sibling module — [`../ll1`](../ll1/README.md), then
...
[`../ll1`](../ll1/README.md): `parser` refuses a grammar that is not LL(1) —
left recursion, a first/first or a first/follow conflict — before any input,
naming the rule. Every builder follows it, each with its own constraint
...
- `fjs/rtti` — a type is a schema *value* from which `ts/` derives the
  ...
  `validate`);
- `fjs/ebnf` — a grammar value from which the builders derive automata.

`rtti`:types :: `ebnf`:grammars :: `html`:markup — define a value, derive
...
2. **Recognizer over LL(1) grammars — context-free.** Largely *subtraction*
   from the existing matcher: [`../ll1`](../ll1/README.md)'s machine already
   walks the rule set on an explicit stack; the recognizer is the same walk
   without building a node or applying a mapping, returning accept/reject +
   final configuration. `S` is a parser configuration (stack), **not** finite.
   This is the tier above the scanner (PL/structure recognition).

**The core is symbol-agnostic; alphabet-specific authoring lives in adapters.**
Both levels — the functional front end and the serializable data IR — are
neutral about whether a symbol originated as a byte, Unicode code point, token
symbol, or some future intermediate alphabet. A range-set terminal assigns no
Unicode or byte meaning to its symbols.

`range` and `set` ship in the front end, whose rule union already reads a
`string` as a Unicode sequence; the text helpers the union does not imply
are [unicode-rules](../unicode/todo/unicode-rules.md)'s, and byte / hex
literals, byte sequences and byte ranges are [`../byte`](../byte/README.md)'s.
They are authoring adapters that lower to ordinary rules before automaton
construction. The recognizer/DFA backends consume the resulting `RuleSet`;
they do not define a second family of binary helpers.
...
- [x] A backend is a sibling module over the serializable IR, not part of
      it: [`../ll1`](../ll1/README.md) is, and new backends land beside it as
      **`fjs/ebnf/recognizer` and `fjs/ebnf/dfa`**.
...
- [`../README.md`](../README.md#logical-eof-in-parser-input) — the logical
  EOF semantics every recognizer backend must preserve.
- [unicode-rules](../unicode/todo/unicode-rules.md) and
  [`../byte`](../byte/README.md) — the Unicode and byte authoring helpers;
