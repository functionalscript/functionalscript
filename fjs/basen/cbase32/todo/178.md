...
`fjs/basen/cbase32/module.f.ts` hand-rolls two pieces of pure bit-vector arithmetic
...
```ts
// fjs/basen/cbase32/module.f.ts:32  — pad to a multiple of 5 bits with a 1-then-0s marker
...
// fjs/basen/cbase32/module.f.ts:39  — strip the marker
...
Add to `fjs/types/bit_vec/module.f.ts` a padding pair (and/or the trailing-zero
...
- Relatedly, `fjs/asn.1/module.f.ts:128` has a `round8` (round a bit length up to
...
- [i167](./README.md), [i168](./README.md) — other `bit_vec`/text bit-level
  extractions.
- `fjs/types/bit_vec/module.f.ts` — proposed home.
