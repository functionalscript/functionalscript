...
array (as `fjs/cas/cli/module.f.mjs` does), it must also export a redundant
...
always go through `dispatch` — no array check needed at the call site:
...
wrapper in `fjs/cas/cli/module.f.mjs` simplifies to `export const main = commands`.

### Tasks

- [ ] Export `NodeMain = NodeProgram | Commands<NodeOp>` from
      `fjs/effects/node/types.ts`.
- [ ] Widen `dispatch` to take a `Program` as well, short-circuiting to it.
- [ ] Widen `runEffect` and `run` in `fjs/effects/node/module.mjs` to accept
      `NodeMain`; they always call `dispatch`, which resolves the union.
- [ ] Simplify `fjs/cas/cli/module.f.mjs`: `export const main = commands` (drop
      the `dispatch` wrapper).
- [ ] Add a proof in `fjs/proof.f.mjs` covering the `Commands`-as-`main` path.

### Related

- `fjs/module.f.mjs` — the `run` command.
- `fjs/cas/cli/module.f.mjs` — the `main = dispatch(commands)` boilerplate this
  issue eliminates.
