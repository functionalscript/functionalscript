...
```ts
// fjs/js/tokenizer/module.f.mjs:194
const union
...
| `range_map.merge` wrapper | `reduce` :48-62 | `rangeMapMerge` :338-344 |
| single-range cell | `codePointRange`/`range` :64-66 | `rangeFunc` :346-348 |
| range-set cell | `rangeSet` :68-84 | `rangeSetFunc` :361-370 |
| build + dispatch | `create` :86-97 | `create` :372-378 |
...
- **Distinct from [i165](../../ebnf/todo/layered-parser.md).** That issue proposes a grammar-driven
...
- [i165](../../ebnf/todo/layered-parser.md) — layered grammar parser (different mechanism).
- `fjs/types/range_map/module.f.mjs` — the underlying structure both consumers wrap.
