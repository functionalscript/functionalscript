...
- `fjs/bnf/data/module.f.ts:104-116` — `data()` (over `DataRule`, adds the
  `'string'` case)
- `fjs/bnf/descent/module.f.ts:67-92` — `emptyTagMapAdd`
- `fjs/bnf/descent/module.f.ts:121-160` — `descentParser`'s inner `f`
- `fjs/bnf/ll1/module.f.ts:116-157` — `dispatchRule`
...
Add a visitor over the data `Rule` ADT in `fjs/bnf/data/module.f.ts` (the
module that owns the type), mirroring the proven `visit` pattern in
`fjs/types/rtti/common`:
...
- [ ] Add `RuleVisitor`/`matchRule` to `fjs/bnf/data/module.f.ts` with proof
      coverage.
