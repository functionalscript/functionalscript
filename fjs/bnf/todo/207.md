...
i143, so instantiation-time grammar checking is a natural
...
`subset` predicate of §5.3, gated on i143; until it lands,
...
i143 exists, lift the shape-compatibility half to grammar
...
- i143 (retired; shipped as [`fjs/types/rtti/data/`](../../types/rtti/data/module.f.mjs)) —
  serializable RTTI data form and its planned `equal`/`subset` algebra; the predicate the instantiation-time boundary check
