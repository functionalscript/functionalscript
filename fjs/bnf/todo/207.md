...
*sound but deliberately incomplete* (`fjs/types/rtti/data/README.md`): it never
...
- **Projection.** `parse` does not only accept a value, it *builds a fresh one
  ...
  more, so lifting the check does not lift the projection: for an open struct
  or tuple, an action that enumerates keys or reads a tuple's length would
  start seeing the extras the per-node `parse` used to strip. Either keep a
  per-node `parse` where `in` is open, or drop normalization from the action
  contract and say so — this design currently assumes it without providing it.
...
where `in` is an open struct or tuple, which `subset` does not replace. Recognize repetition by **structural right-recursion analysis**
