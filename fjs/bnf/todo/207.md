...
2. **Type safety.** The action's input must match what the children actually
   ...
   schema (`fjs/types/rtti`) and check the boundary at runtime (§5).
...
RTTI (`fjs/types/rtti`) already gives us exactly the missing piece: a runtime
...
`fjs/types/rtti/parse` (`parse(schema)(value)` — builds a fresh value holding
...
[`fjs/types/rtti/data/module.f.mjs`](../../types/rtti/data/module.f.mjs). This
...
One limitation the original text did not anticipate, which the design must now
settle: the shipped `subset` is **sound but incomplete**. It never answers `true`
for a non-inclusion, but it may answer `false` for an inclusion that holds only
by distributing a union across positions, or whose left side is a non-syntactic
empty set (`fjs/types/rtti/data/README.md`). A gate that rejects a grammar on
`false` would therefore reject some legitimate grammars, so §5.3 has to choose:
reject on `false`, or treat `false` as *unproven* and fall back to the §5.2
per-node form for that rule.
...
the recommended target; the §5.2 per-node form remains the fallback for rules
where `subset` cannot prove the inclusion.
...
§5.3 removes the per-node cost for every rule whose boundary the `subset`
predicate can prove — not outright: a rule it answers `false` for keeps the
per-node form above, since `subset` is sound but incomplete (§5.3). The
...
`subset` predicate of §5.3, which has shipped; the per-node §5.2 form remains
the fallback where that predicate cannot prove the inclusion, and name
resolution can still be checked at instantiation independently.
...
O(actioned rules) once, keeping the per-node §5.2 fallback for the rules it
cannot prove and still resolving names at construction. Recognize repetition by **structural right-recursion analysis**
...
- i172 (retired; shipped as [`fjs/types/rtti/validate/`](../../types/rtti/validate/module.f.mjs)
  and [`fjs/types/rtti/parse/`](../../types/rtti/parse/module.f.mjs)) — `validate`/`parse`
