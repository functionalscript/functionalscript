...
> §5.2; and the instantiation-time `subset` check §5.3, gated on
> [rtti schema-algebra](../../types/rtti/todo/schema-algebra.md)). Before
...
A functional grammar rule (`fjs/bnf/module.f.ts`) is one of four shapes, behind
...
`parser`/`descentParser` (`fjs/bnf/data/module.f.ts`) produce a generic AST:
...
- **one grammar definition** — the functional `Rule` form (`fjs/bnf/module.f.ts`);
...
`fjs/types/rtti/validate` (`validate(schema)(value) → Result`) and
`fjs/types/rtti/parse` (`parse(schema)(value)` — builds a fresh, closed value)
...
[rtti schema-algebra](../../types/rtti/todo/schema-algebra.md), so instantiation-time grammar checking is a natural
*consumer* of that algebra and should be gated on it.
...
input that reaches its branch. Recommended target once the RTTI `subset`
predicate lands; until then,
...
`subset` predicate of §5.3, gated on [rtti schema-algebra](../../types/rtti/todo/schema-algebra.md); until it lands,
...
[rtti schema-algebra](../../types/rtti/todo/schema-algebra.md) exists, lift the shape-compatibility half to grammar
...
- [rtti schema-algebra](../../types/rtti/todo/schema-algebra.md) — the planned
  `equal`/`subset` algebra; the predicate the instantiation-time boundary check
  (§5.3) depends on.
- [rtti data form](../../types/rtti/todo/data-form.md) — the serializable,
  function-free RTTI form that algebra is built on, itself modelled on
  `fjs/bnf/data`; relevant if schemas are auto-derived from the BNF data form.
