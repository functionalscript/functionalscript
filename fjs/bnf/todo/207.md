...
> implementation it must first be rebased on the alphabet-specific BNF split so
...
> produce a flat AST — has **shipped** for `bnf/data` and `bnf/descent`; see
> [`fjs/bnf/data/README.md`](../data/README.md#the-repeat-rule) and
> [ll1-ast-divergence](./ll1-ast-divergence.md) for the backend that is still
> nested — and whose AST an action cannot address at all, since a rule entered
> through a dispatch there has no node to attach one to. The opt-in for the *ambiguous* cases (list vs. right-associative tree)
