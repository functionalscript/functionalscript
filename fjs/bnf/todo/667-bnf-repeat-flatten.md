...
is no primitive `repeat` rule. The data `Rule` (in `fjs/bnf/data/module.f.ts`)
...
`fjs/bnf/descent/module.f.ts`'s `f` matcher used to mirror this
...
- [ ] Add `Repeat = string` and extend `Rule` in `fjs/bnf/data/module.f.ts`
...
- `fjs/bnf/data/module.f.ts` — the data `Rule` type and all dispatch sites
- `fjs/bnf/module.f.ts` — BNF combinators including `repeat0Plus`
