...
never throws at all — recoverable failure is expressed with `Result` (`fjs/types/result`), which
...
- [ ] Move `assertThrowsCustomMsg`, `assertEqThrowsOnUnequal`, `assertEqThrowsOnUnequal3`,
      `todoThrows` (`fjs/asserts/`) and `unwrapError` (`fjs/types/result/`) into a sibling
