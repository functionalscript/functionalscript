...
nothing else.** `website/module.f.mjs` selects on the extension, so the
proof list it writes into a page contains only those modules. That is the design, not a first
