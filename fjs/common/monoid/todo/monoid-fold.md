...
- `fjs/types/number/module.f.ts:11-12` — `sum = reduce(addition)(0)`
- `fjs/types/bigint/module.f.ts:48-49` — `sum = reduce(addition)(0n)`
- `fjs/types/bigint/module.f.ts:65-66` — `product = reduce(multiple)(1n)`
- `fjs/types/string/module.f.ts:23-30` — `concat = reduce(concatOp)` where
...
`fjs/common/monoid/module.f.ts`. That module ships `repeat`, which consumes
...
- [ ] Add `fold` to `fjs/common/monoid/module.f.ts` with proof coverage.
...
- `fjs/common/monoid/module.f.ts` — `repeat`, the existing `Monoid` consumer.
- `fjs/types/prime_field/module.f.ts`, `fjs/types/bit_vec/module.f.ts` —
