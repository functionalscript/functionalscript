...
// down as a check here, and the combinator signatures with them.
//
// Those signatures were `./proof.f.mjs`'s `signatures` entry, downstream of
// the implementation they verify. Its body was nothing but typedefs, so none
// of them bound to a statement and all eight were green whatever they claimed
// (`../AGENTS.md` §1.4). A module-scope alias is resolved on sight wherever it
// sits, and here it reuses the instantiation the rules below already set up
// rather than declaring a second `_AddOp` / `_MulOp` pair; the combinators
// arrive through an `import type`, which is erased like every other import in
// this file.
...
// unions only the operation sets. It is the layer's primitive — `step` and
// `catchStep` are it with a tag test in front — so this signature is the one
// the other two are derived from rather than a third variant beside them.
