...
export type MemCreate = readonly['memCreate', <T>(value: T) => OpResult<Key<T>>]

/** Reads the current value stored at `key`. */
export type MemRead = readonly['memRead', <T>(key: Key<T>) => OpResult<T>]

/** Replaces the current value stored at `key`. */
export type MemWrite = readonly['memWrite', <T>(key: Key<T>, value: T) => OpResult<void>]

export type MemOp = MemCreate | MemRead | MemWrite

/**
 * The storage the synchronous memory interpreter threads through a run:
 * the counter the next key is minted from, and every slot by key.
 */
export type MemoryState = {
    readonly next: number
    readonly values: StringMap<unknown>
}
