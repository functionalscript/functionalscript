...
type WrappedEff<O extends Operation, T> = {
    readonly value: Effect<O, T>
    readonly step: <Q extends Operation, R>(f: (t: T) => Eff<Q, R>) => WrappedEff<O | Q, R>
}

const unwrap = <O extends Operation, T>(result: Eff<O, T>): Effect<O, T> =>
 ...
 */
export type Eff<O extends Operation, T> = WrappedEff<O, T> | Effect<O, T>

/** Wraps a raw {@link Effect}; the bridge into the `Eff` world. */
export const eff = <O extends Operation, T>(value: Effect<O, T>): WrappedEff<O, T> => ({
    value,
    step: f => eff(step(value, t => unwrap(f(t)))),
})

/** The monad unit: a pure value as an `Eff`. */
export const pure = <T>(v: T): WrappedEff<never, T> => eff(pureEffect(v))
