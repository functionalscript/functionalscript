...
(`fjs/cas/cli/module.f.ts`) and the MCP server (`fjs/mcp/module.f.ts`) —
...
reviewed design document (in `fjs/cas/README.md` or a dedicated doc under
`fjs/cas/`); implementation is then re-filed as follow-up issues sized from
the design.
...
2. **The command set** — today's `add`, `get`, `list`; planned commands
   ...
   [`fjs/cas/evo`](../evo/), rather than folded into this command set — a
...
3. **The exposure matrix** — which transports (CLI, MCP,
   ...
   `fjs/mcp/README.md`); the same rule presumably binds a Web API.
4. **How one declaration drives every adapter.** An idea to evaluate, not
   ...
   option parsing ([fjs/cli options-edsl](../../cli/todo/options-edsl.md))
...
5. **Where the shared layer lives** (e.g. `fjs/cas/commands/`) and its
...
- [fjs/cli options-edsl](../../cli/todo/options-edsl.md) — the CLI-side
...
- [web-api-server](./web-api-server.md) — the third transport this design
  must anticipate.
- [`fjs/cas/evo`](../evo/) — implemented as its own MCP server rather than
