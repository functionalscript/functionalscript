...
fn unary_plus<A: Any>() {
    ...
    for (a, expected, test_case) in test_cases.iter() {
        test_op::<A>(Any::unary_plus(a.clone()).unwrap(), expected.clone(), test_case);
    }
...
fn multiply<A: Any>() {
    ...
    let test_cases: Vec<(A, A, A, &str)> = vec![
        ...
        (n10.clone(), n10.clone(), Simple::Number(100.0).to_unknown(), "10 by 10"),
        (n_minus10.clone(), n10.clone(), Simple::Number(-100.0).to_unknown(), "-10 by 10"),
        (bi0.clone(), bi0.clone(), bi0.clone(), "0n by 0n"),
        (bi0.clone(), bi1.clone(), bi0.clone(), "0n by 1n"),
        (bi1.clone(), bi1.clone(), bi1.clone(), "1n by 1n"),
        (bi1.clone(), bi_minus1.clone(), bi_minus1.clone(), "1n by -1n"),
        (bi1.clone(), bi10.clone(), bi10.clone(), "1n by 10n"),
        (bi_minus1.clone(), bi10.clone(), A::BigInt::new(Sign::Negative, [10]).to_unknown(), "-1n by 10n"),
        (bi10.clone(), bi10.clone(), A::BigInt::new(Sign::Positive, [100]).to_unknown(), "10n by 10n"),
        (bi_minus10.clone(), bi10.clone(), A::BigInt::new(Sign::Negative, [100]).to_unknown(), "-10n by 10n"),
        ...
        ([n10.clone()].to_array_unknown(), n1.clone(), n10.clone(), "[10] by 1"),
        (["10".to_unknown()].to_array_unknown(), n1.clone(), n10.clone(), "[\"10\"] by 1"),
        ([n0.clone(), n0.clone()].to_array_unknown(), n1.clone(), nan.clone(), "[\"0,0\"] by 1"),
        ([].to_object_unknown(), n1.clone(), nan.clone(), "{{}} by 1"),
        // TODO: decide on testing objects with valueOf, toString functions.
    ];
    for (a, b, expected, test_case) in test_cases.iter() {
        test_op::<A>(Any::multiply(a.clone(), b.clone()).unwrap(), expected.clone(), test_case);
        test_op::<A>(Any::multiply(b.clone(), a.clone()).unwrap(), expected.clone(), test_case);
    }
}
