...
| `.` / `[]` | Member access       | [x]      | [`any/member_access.rs`](src/vm/any/member_access.rs) — `Any::member_access` dispatches to [`array/member_access.rs`](src/vm/array/member_access.rs), [`string/member_access.rs`](src/vm/string/member_access.rs), and [`object/member_access.rs`](src/vm/object/member_access.rs) (staged as [`todo/member-access-operator.md`](todo/member-access-operator.md)); `Number`/`Boolean`/`BigInt`/`Function` have no own properties, so every key on one answers `undefined`; still no prototype chain or built-in methods (`.map`, `.push`, `.slice`, getters), which is out of scope — see the plan's Non-goals |
| `in`       | Property check      | [ ]      | |
