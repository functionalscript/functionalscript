...
| `.` / `[]` | Member access       | [ ]      | [`any/member_access.rs`](src/vm/any/member_access.rs) — `Any::member_access` dispatches to [`array/member_access.rs`](src/vm/array/member_access.rs) and [`string/member_access.rs`](src/vm/string/member_access.rs) for `Array<A>`/`String<A>` indexing and `.length` (Stages 1–2 of [`todo/member-access-operator.md`](todo/member-access-operator.md)); `Object<A>`/prototype-chain generalization (Stage 3) not yet |
| `in`       | Property check      | [ ]      | |
