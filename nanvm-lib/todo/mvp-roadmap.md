...
and proven with a named-only module and its dependency. The harness selects
an export explicitly: `run(module, export, action)` in
[`nanvm-harness`](../../nanvm-harness/src/lib.rs) reads the named export or
calls it with supplied arguments, and the acceptance test selects and calls
`main` through it. There is no CLI; a CLI is its own task once a use for it
exists.
...
- [x] **Named-module integration** — harness export selection:
      `run(module, export, action)` in `nanvm-harness`.
      ...
      JavaScript, both EDAG evaluators and generated Rust, where the harness
      selects and calls it. Keep all exports in the module result; no default-export
