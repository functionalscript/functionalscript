...
| `toExponential(f)` | `f` `undefined`, absent or passed, is kept apart: it means the shortest form below, not `0`, so `(1.5).toExponential(undefined)` is `"1.5e+0"`. Otherwise `ToIntegerOrInfinity(f)` first, so `NaN` is `0` and a bigint is a `TypeError` even on `Infinity`. Then a non-finite number is its `ToString`, checked **before** the range, so `Infinity.toExponential(101)` is `"Infinity"`. Then `[0, 100]`, else a `RangeError`. `f` `undefined` means as many digits as the shortest round-tripping representation, the digits `ToString` uses. Otherwise `f + 1` significant digits of the magnitude, the larger on a tie, with the sign put back, so `(-2.5).toExponential(0)` is `"-3e+0"`; `-0` is unsigned, so `(-0).toExponential(2)` is `"0.00e+0"`. Exponent written `e+N` / `e-N`. |
| `toPrecision(p)` | `p` `undefined` is `ToString`. Otherwise `ToIntegerOrInfinity(p)` first, so `"2.9"` is `2`; then a non-finite number is its `ToString`; then `[1, 100]`, else a `RangeError`. `p` significant digits of the magnitude, the larger on a tie, with the sign put back, so `(-2.5).toPrecision(1)` is `"-3"`, and `-0` is unsigned, so `(-0).toPrecision(3)` is `"0.00"`; exponential notation when the exponent is below `-6` or at least `p`. |
