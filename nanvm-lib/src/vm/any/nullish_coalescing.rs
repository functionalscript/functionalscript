...
impl<A: IVm> Any<A> {
    ...
    /// `logical_and`/`logical_or`, and `rhs` is a thunk for the same reason
    /// theirs is: the right operand is established only if `self` is
    /// nullish.
    ///
    /// Establishes `rhs` and returns its value if `self` is `null` or
    /// `undefined`, otherwise returns `self` unchanged — unlike
