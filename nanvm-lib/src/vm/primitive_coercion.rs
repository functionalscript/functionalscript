...
pub trait PrimitiveCoercion<A: IVm> {
    ...
    fn coerce_to_primitive(&self, preferred_type: ToPrimitivePreferredType) -> Result<Any<A>, Any<A>>;
}
...
impl<A: IVm> PrimitiveCoercion<A> for bool {
    fn coerce_to_primitive(&self, preferred_type: ToPrimitivePreferredType) -> Result<Any<A>, Any<A>> {
        match preferred_type {
            ToPrimitivePreferredType::Number => Ok(Unpacked::Number(self.coerce_to_number()?).into()),
            ToPrimitivePreferredType::String => Ok(Unpacked::String(self.coerce_to_string()?).into()),
        }
    }
}
