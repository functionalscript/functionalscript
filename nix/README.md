...
**The middle line is a guard, not a courtesy.** Every other generated file lands
under `nix/` or `.github/workflows/`, directories this generator owns; `dev.sh`
is the first it writes into one it does not, and `dev.sh` is a name a project is
likely to have already. So the generator reads the file first and only writes
over one carrying that marker — an unmarked `dev.sh` is left alone, with a line
on `stderr` saying so. The marker rather than the whole text, because an older
generated script differs from the current one and is still ours to replace,
while a script that happens to open a Nix shell is still yours to keep.
